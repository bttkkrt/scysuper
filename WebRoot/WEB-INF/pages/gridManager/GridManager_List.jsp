...
<title>乡镇街道列表</title>
	...
	<script>
        ...
        function view(row_Id,name){
        	...
        	var id  = "town_list";
        	var text = "绑定"+name+"信息";
        	var url = "${ctx}/jsp/gridManager/gridManagerBindFgld.action?deptCode="+row_Id;
    		window.parent.addTab(id,text,url);
        }
        function mapView(row_Id,name){
        	var id  = "town_list_map";
        	var text = "查看"+name+"下地图信息";
        	var url = "${ctx}/jsp/gridManager/gridManagerTownMapView.action?deptCode="+row_Id;
    		window.parent.addTab(id,text,url);
        }
        function close_win(){
        ...
        function search_gridManager(){
        	var queryParams = {
					"gridManager.upUserid": $("#upUserid").val()
			};        	
        ...
        $(function(){
			...
			$('#pagination').datagrid({
				...
				remoteSort: false,
				columns:[[
				          {field:'name',title:'乡镇名称',width:200},
				          {field:'count',title:'有证照企业数',width:200},
				          {field:'wzCount',title:'无证照企业数',width:200},

				          {field:'op',title:'操作',width:100,formatter:function(value,rec){
	                             return "<span style='color:red;cursor:hand' onclick=\"view('"+rec.code+"','"+rec.name+"')\">绑定</span>"+
	                             "&nbsp;&nbsp;&nbsp;&nbsp;<span style='color:blue;cursor:hand' onclick=\"mapView('"+rec.code+"','"+rec.name+"')\">地图查看</span>";
                          }}
				        ]],
				pagination:true,
...
<form name="myform" method="post">
	<div class="submitdata">
		<table width="100%">
			<tr>
				<th width="15%">所属街道</th>
				<td width="35%"><cus:SelectOneTag property="gridManager.upUserid" defaultText='请选择' codeName="相城地址" value="${gridManager.upUserid}" /></td>
			</tr>
			<tr>
