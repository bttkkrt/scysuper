...
<title>中队中队长网格管理</title>
	...
	<script>
        ...
        function view(row_Id,name){
        	var id  = "town_list";
        	var text = "绑定到"+name+"的镇级中队安监员信息";
        	var url = "${ctx}/jsp/gridManager/gridManagerBindTownUser.action?userId="+row_Id+"&deptCode=${deptCode}";
    		window.parent.addTab(id,text,url);
        }
         function mapView(row_Id,name){
        	var id  = "town_list_map";
        	var text = "查看"+name+"下地图信息";
        	var url = "${ctx}/jsp/gridManager/gridManagerTownDZMapView.action?deptCode="+row_Id;
    		window.parent.addTab(id,text,url);
        }
        function close_win(){
        ...
        $(function(){
			$('#pagination').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'gridManagerBindTownDzQuery.action',
				queryParams:{
				...
				columns:[[
				          {field:'name',title:'中队中队长',width:200},
				          ...
				          {field:'op',title:'操作',width:200,formatter:function(value,rec){
	                             return "<span style='color:red;cursor:hand' onclick=\"view('"+rec.id+"','"+rec.name+"')\">绑定镇级中队安监员</span>"+
