...
<title>镇级中队安监员网格管理</title>
	...
	<script>
        ...
        function view(row_Id,name){
        	var id  = "town_list";
        	var text = "绑定到"+name+"的村级安全员信息";
        	var url = "${ctx}/jsp/gridManager/gridManagerCountryUser.action?userId="+row_Id+"&deptCode=${deptCode}";
    		window.parent.addTab(id,text,url);
        }
         function mapView(row_Id,name){
        	var id  = "town_list_map";
        	var text = "查看"+name+"下地图信息";
        	var url = "${ctx}/jsp/gridManager/gridManagerTownAQYMapView.action?deptCode="+row_Id;
    		window.parent.addTab(id,text,url);
        }
        function close_win(){
        ...
        $(function(){
			$('#pagination').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'gridManagerBindTownUserQuery.action',
				queryParams:{
				...
				columns:[[
				          {field:'name',title:'镇级中队安监员',width:200},
				          ...
				          {field:'op',title:'操作',width:200,formatter:function(value,rec){
	                             return "<span style='color:red;cursor:hand' onclick=\"view('"+rec.id+"','"+rec.name+"')\">绑定村级安全员</span>"+
