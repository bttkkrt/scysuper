...
<title>网格管理</title>
	...
	<script>
        ...
        function mapView(row_Id,name){
        	...
        	var text = "查看"+name+"下无证企业地图信息";
        	var url = "${ctx}/jsp/gridManager/gridManagerCompanyMapView.action?deptCode="+row_Id;
    		window.parent.addTab(id,text,url);
        }
        function close_win(){
        ...
        function bindCompanyUser(){
			...
			if(rows.length<1){
			    ...
			    $.messager.confirm("删除","确定要绑定吗?",function(result){
			        if(result){
		                $.ajax({
		                	url : "gridManagerBindNoCompanyUser.action",
		                	type: 'post',
		                    dataType: 'json',
		                    async : false,
		                    data:{ 
        ...
        function unbindCompanyUser(){
			...
			if(rows.length<1){
			    ...
			    $.messager.confirm("删除","确定要解除绑定吗?",function(result){
			        if(result){
		                $.ajax({
		                	url : "gridManagerJcBindNoCompanyUser.action",
		                	type: 'post',
		                    dataType: 'json',
		                    async : false,
		                    data:{ 
        ...
        $(function(){
			$('#pagination').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'gridManagerBindNoCompanyUserQuery.action',
				queryParams:{
				...
				columns:[[
				          {field:'name',title:'无证企业名称',width:300},
				    ...
				    {field:'op',title:'操作',width:300,formatter:function(value,rec){
	                             var opt = "<span style='color:blue;cursor:hand' onclick=\"mapView('"+rec.id+"','"+rec.name+"')\">地图查看</span>";
                          		return opt;
                          }}
				        ]],
				pagination:true,
		...
		<table width="100%">
			...
			<tr>
				<td colspan="4" align="center" style="text-align:center;">
					...
					<a href="###" class="easyui-linkbutton" onclick="bindCompanyUser();" iconCls="icon-add">绑定</a>&nbsp;
					<a href="###" class="easyui-linkbutton" onclick="unbindCompanyUser();" iconCls="icon-add">解除绑定</a>&nbsp;					
				</td>
			</tr>
		</table>
	</div>
