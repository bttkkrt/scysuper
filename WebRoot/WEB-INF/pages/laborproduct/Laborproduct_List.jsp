...
<html>
<head>
<title>LABORPRODUCT管理</title>
    <%@include file="/common/jsLib.jsp"%>
	<script>
        
        function addNew(){
            window.open("${ctx}/jsp/laborproduct/laborproductInitEdit.action?flag=add");
        }
        function edit(row_Id){
            window.open("${ctx}/jsp/laborproduct/laborproductInitEdit.action?flag=mod&laborproduct.id="+row_Id);
        }
        function view(row_Id){
        	window.open("${ctx}/jsp/laborproduct/laborproductView.action?laborproduct.id="+row_Id);
        }
        function close_win(){
        ...
        function search_laborproduct(){
        	var queryParams = {
				"laborproduct.deptId": $("#deptId").val(),
					"laborproduct.qymc": $("#qymc").val(),
 ...
 "createTimeStart" :$("#createTimeStart").val(),
			 "createTimeEnd" :$("#createTimeEnd").val()
				...
				columns:[[
				{field:'szz',title:'所在镇',width:fixWidth(0.25)},
				          {field:'qymc',title:'企业名称',width:fixWidth(0.25)},
            {field:'createTime',title:'上传时间',width:fixWidth(0.25),formatter:function(value,rec){
...
<form name="myform" method="post">
	<div class="submitdata">
		<table width="100%">
			<tr>
				<th width="15%"> 企业名称</th>
				<td width="35%"><input name="laborproduct.qymc" id="qymc" value="${laborproduct.qymc}" type="text"></td>
			...
			<s:if test="visable==1">
				<tr>
				<th width="15%">所属地区</th>
				<td width="35%"><cus:SelectOneTag property="county" defaultText='请选择' codeName="地址"  onchange="querySzz(this.value);"  style="width:150px;" />
					<select id="deptId" name="laborproduct.deptId" style="width:150px;" ><option value="">请选择</option></select>
						
					<!--hanxc 2014/11/11 		
					<cus:SelectOneTag property="laborproduct.deptId" defaultText='请选择' codeName="相城地址" value="${laborproduct.deptId}" /></td>
					 -->
				</tr>
				</s:if>
			<tr>
			...
			<tr>
				...
				<a href="###" class="easyui-linkbutton" onclick="search_laborproduct()" iconCls="icon-search">查询</a>&nbsp;
