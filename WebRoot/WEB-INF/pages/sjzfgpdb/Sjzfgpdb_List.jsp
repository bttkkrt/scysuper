...
<head>
<title>三级政府挂牌督办重大隐患整改管理</title>
    <%@include file="/common/jsLib.jsp"%>
	<script>
        
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_sjzfgpdb","添加三级政府挂牌督办重大隐患整改","${ctx}/jsp/sjzfgpdb/sjzfgpdbInitEdit.action?flag=add&dt="+dt.getTime(),800,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_sjzfgpdb","修改三级政府挂牌督办重大隐患整改","${ctx}/jsp/sjzfgpdb/sjzfgpdbInitEdit.action?flag=mod&sjzfgpdb.id="+row_Id+"&dt="+dt.getTime(),800,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_sjzfgpdb","查看三级政府挂牌督办重大隐患整改","${ctx}/jsp/sjzfgpdb/sjzfgpdbView.action?sjzfgpdb.id="+row_Id+"&dt="+dt.getTime(),800,500);
        ...
        function exportData()
        {
          var queryMonthTimeStart = $("#queryMonthTimeStart").val();
        	...
        	document.myform.action = "${ctx}/jsp/sjzfgpdb/sjzfgpdbExport.action";
...
"sjzfgpdb.areaName": $("#areaName").val()
        ...
        $(function(){
        	$('#pagination').datagrid($.extend(window.dg_cm_pp,{
				title:'三级政府挂牌督办重大隐患整改列表',
				url:'sjzfgpdbQuery.action',
				queryParams:{
					 "queryMonthTimeStart" :$("#queryMonthTimeStart").val(),
 "queryMonthTimeEnd" :$("#queryMonthTimeEnd").val(),
"sjzfgpdb.areaName": $("#areaName").val()
			...
			<tr>
				...
				<tr>
					<td colspan="4" align="center">
						<a href="###" class="btn_01" onclick="search_sjzfgpdb()" >查询<b></b></a>&nbsp;
