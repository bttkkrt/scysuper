...
<html>
<head>
<title>职业病危害预评价报告表备案通知书管理</title>
    <%@include file="/common/jsLib.jsp"%>
	<script>
          var flag = "${flag}";
        var createUserID = "${createUserID}";
        function addNew(){
        	window.open("${ctx}/jsp/occupationalEvaluatePlan/occupationalEvaluatePlanInitEdit.action?flag=add");
        }
        function edit(row_Id){
        	window.open("${ctx}/jsp/occupationalEvaluatePlan/occupationalEvaluatePlanInitEdit.action?flag=mod&occupationalEvaluatePlan.id="+row_Id);
        }
        function view(row_Id){
        	window.open("${ctx}/jsp/occupationalEvaluatePlan/occupationalEvaluatePlanView.action?occupationalEvaluatePlan.id="+row_Id);
        }
        function printOut(row_Id)
        {
        	window.open("${ctx}/jsp/occupationalEvaluatePlan/occupationalEvaluatePlanPrint.action?occupationalEvaluatePlan.id="+row_Id);
        }
        function close_win(){
        ...
        function reloadDate(){
        	...
        	search_occupationalEvaluatePlan();
        }
        function del(){
			...
			if(rows.length<1){
			    ...
			    $.messager.confirm("删除","确定要删除吗?",function(result){
			        if(result){
		                $.ajax({
		                	url : "occupationalEvaluatePlanDel.action",
		                	type: 'post',
		                    dataType: 'json',
		                    async : false,
		                    data:{ 
        ...
        function search_occupationalEvaluatePlan(){
        	var queryParams = {
        		"occupationalEvaluatePlan.szzid": $("#szzid").val(),
				"occupationalEvaluatePlan.companyName": $("#companyName").val(),
 ...
 "queryTbrqEnd" :$("#queryTbrqEnd").val()
        ...
        $(function(){
			...
			$('#pagination').datagrid({
				title:'职业病危害预评价报告表备案通知书列表',
				iconCls:'icon-save',
				nowrap: false,
				striped: true,
				collapsible:true,
				url:'occupationalEvaluatePlanQuery.action',
				queryParams:{
					"occupationalEvaluatePlan.szzid": $("#szzid").val(),
				"occupationalEvaluatePlan.companyName": $("#companyName").val(),
				"queryTbrqStart" :$("#queryTbrqStart").val(),
...
{field:'sendTime',title:'回执时间',width:fixWidth(0.2)},
	...
	<div class="submitdata">
		<table width="100%">
			<tr>
				...
				<td width="35%">
					...
					<cus:SelectOneTag property="dangerousChemicalsPlan.szzid" defaultText='请选择' codeName="相城地址" value="${dangerousChemicalsPlan.szzid}" /></td>
					 -->
				<th width="15%">单位名称</th>
				<td width="35%"><input name="occupationalEvaluatePlan.companyName" id="companyName" value="${occupationalEvaluatePlan.companyName}" type="text"></td>
			</tr>
			<tr>
				<th width="15%">回执时间</th>
			...
			<tr>
				...
				<a href="###" class="easyui-linkbutton" onclick="search_occupationalEvaluatePlan()" iconCls="icon-search">查询</a>&nbsp;
