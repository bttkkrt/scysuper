...
<title>WHP_CLSC管理</title>
	...
	<script>
        ...
        function del(){
        	var rows = document.getElementYxqyName("xxx");
        	var ids = "";
        	var len = rows.length;
			for(var i=0;i<len;i++){
				if(rows[i].checked){
        ...
        function search_whpClsc(){
        	var queryParams = {
        		"whpClsc.fzjg":$("#fzjg").val(),
					"whpClsc.szzid": $("#szzid").val(),
					"whpClsc.qymc": $("#qymc").val(),
					"whpClsc.state": $("#state").val(),
			"createTimeStart" :$("#createTimeStart").val(),
			 "createTimeEnd" :$("#createTimeEnd").val(),
			 "createTimeStartBa" :$("#createTimeStartBa").val(),
			 "createTimeEndBa" :$("#createTimeEndBa").val(),
			 "createTimeStartYxq" :$("#createTimeStartYxq").val(),
			 "createTimeEndYxq" :$("#createTimeEndYxq").val(),
			 "whpClsc.sclx": "9"
			 ...
			 "whpClsc.sclx": "9"
				...
				columns:[[
					 	{field:'szzname',title:'所属地区',width:fixWidth(0.1)},
						  ...
						  {field:'barq',title:'备案日期',width:fixWidth(0.1),formatter:function(value,rec){
								if(rec.barq==null) {return;}
								var date = new Date(rec.barq.time);
								var month = parseInt(date.getMonth()+1);
								return date.getFullYear()+'年'+month+'月'+date.getDate()+'日  ';
							}},
							{field:'yxq',title:'有效期',width:fixWidth(0.1),formatter:function(value,rec){
								if(rec.yxq==null) {return;}
								var date = new Date(rec.yxq.time);
								var month = parseInt(date.getMonth()+1);
								return date.getFullYear()+'年'+month+'月'+date.getDate()+'日  ';
							}},
	...
	<div class="submitdata">
		<table width="100%">
			...
			<tr>
				...
				<th width="15%">发证机关 </th>
				<td width="35%">
				<input name="whpClsc.fzjg"  id="fzjg" value="${whpClsc.fzjg}" size= 50 type="text"   maxlength="255">
				...
				<tr>
				<th width="15%">备案日期</th>
				<td width="35%"><input name="createTimeStartBa" id="createTimeStartBa" value="${createTimeStartBa}" type="text" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'createTimeEndBa\')}'})" >
					-<input name="createTimeEndBa" id="createTimeEndBa" value="${createTimeEndBa}" type="text" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'createTimeStartBa\')}'})" ></td>
		 
				<th width="15%">有效期</th>
				<td width="35%"><input name="createTimeStartYxq" id="createTimeStartYxq" value="${createTimeStartYxq}" type="text" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'createTimeEndYxq\')}'})" >
					-<input name="createTimeEndYxq" id="createTimeEndYxq" value="${createTimeEndYxq}" type="text" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'createTimeStartYxq\')}'})" ></td>
