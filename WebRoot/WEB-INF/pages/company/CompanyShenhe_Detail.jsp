...
<html>
<head>
	<title><s:if test="type eq 0">查看</s:if><s:elseif test="company.basePass!=1&&type==1">审核</s:elseif><s:else>修改</s:else></title>
	...
	<script type="text/javascript">
		...
		function savePass(){
			debugger;
			var basePass = document.getElementById('basePass').value;
			if(basePass == '2')
			{
				var baseRemark = document.getElementById('baseRemark').value;
				if(baseRemark == '')
				{
					layer.alert("请填写不通过理由");
				}
				else
				{
					document.myform.action="companySaveBase.action";
			...
			else
			{
				...
				if(Validator.Validate(document.myform,3)){
					...
					document.myform.action="companySaveBase.action?pageNumber="+"${pageNumber}";
					document.myform.submit();
				}
			}
		}
 ...
 function querySzc(obj)
				...
				success:function(json){
					...
					selectContainer.append(option); 
	  				for(var i=0; i<json.length; i++){
	  					...
	  					//layer.msg("${company.dwdz1}="+${company.dwdz1});
	...
	function checkRadio(){
		$(".filetype44").change(function(){  
			var val = $("input[name='file44']:checked").val(); //获得选中的radio的值             
			if(val=='1'){    
				document.getElementById("file51").checked=true;
		   		document.getElementById("file50").checked=false;
		   		$('#ifzsqyid').css("display","block"); 
...
<%@include file="/WEB-INF/template/content_title.jsp"%>
	<form name="myform" method="post">
		...
		<input name="company.zcCode" value="${company.zcCode}" type="hidden">
		...
		<div class="submitdata" style="overflow-y: scroll;overflow:auto">
			<table width="100%">
				...
				<tr>
					...
					<td width="35%"><input name="company.tyshxydm" value="${company.tyshxydm}" class="text" size= 50   onKeyUp="validate(event,obj)" dataType="Require" msg="此项为必填" type="text"  maxlength="255"></td>
				</tr>
				<tr>
				...
				<tr>
					<th width="15%">是否直属企业<span style="color:red">*</span></th>
					<td width="35%">			
 ...
 --%>				</tr>
				...
				<tr>
					...
					<!-- 超级管理员或市级用户看到的审核信息 start -->
					<%-- <c:if test="${deptCodeLenth==3||deptCodeLenth==6||deptCodeLenth==9}">
						<c:if test="${ifzsqy==0}">
							<tr>
						...
						<tr>
							<th width="15%" style="color:red"></th>
							<td width="100%" colspan="4">
								<font style="color:red">审核状况:</font><cus:SelectOneTag property="company.basePass" defaultText='请选择' codeName="审核结果"  dataType="Require" msg="此项为必选" /><font color="color:red">*</font>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<font style="color:red">备注:</font><input id="baseRemark"
									name="company.baseRemark" 
									style="width: 50%">
					...
					<!-- 县级用户看到的审核信息 start -->
					<c:if test="${ifzsqy==0}">
						<c:if test="${deptCodeLenth==12}">
							<tr>
								<th width="15%" style="color:red"></th>
								<td width="100%" colspan="4">
									<font style="color:red">县级审核:</font><cus:SelectOneTag property="company.basePass" defaultText='请选择' codeName="审核结果"  dataType="Require" msg="此项为必选" /><font color="color:red">*</font>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<font style="color:red">备注:</font><input id="baseRemark"
										name="company.baseRemark"  
										style="width: 50%">
				...
				<tr>
					<td colspan="4" height="100px" style="text-align:center">
						<a href="#" class="easyui-linkbutton" onclick="savePass();" iconCls="icon-add">确定</a>&nbsp;
