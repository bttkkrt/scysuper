...
<html>
<head>
<title>危险化学品重大危险源备案告知书管理</title>
    <%@include file="/common/jsLib.jsp"%>
	<script>
         var flag = "${flag}";
        var createUserID = "${createUserID}";
        function addNew(){
        	window.open("${ctx}/jsp/dangerousChemicalsPlan/dangerousChemicalsPlanInitEdit.action?flag=add");
        }
        function edit(row_Id){
        	window.open("${ctx}/jsp/dangerousChemicalsPlan/dangerousChemicalsPlanInitEdit.action?flag=mod&dangerousChemicalsPlan.id="+row_Id);
        }
        function view(row_Id){
        	window.open("${ctx}/jsp/dangerousChemicalsPlan/dangerousChemicalsPlanView.action?dangerousChemicalsPlan.id="+row_Id);
        }
        function printOut(row_Id)
        {
        	window.open("${ctx}/jsp/dangerousChemicalsPlan/dangerousChemicalsPlanPrint.action?dangerousChemicalsPlan.id="+row_Id);
        }
        function close_win(){
        ...
        function del(){
			...
			if(rows.length<1){
			    ...
			    $.messager.confirm("删除","确定要删除吗?",function(result){
			        if(result){
		                $.ajax({
		                	url : "dangerousChemicalsPlanDel.action",
		                	type: 'post',
		                    dataType: 'json',
		                    async : false,
		                    data:{ 
        ...
        function search_dangerousChemicalsPlan(){
        	var queryParams = {
        	"dangerousChemicalsPlan.szzid": $("#szzid").val(),
					"dangerousChemicalsPlan.companyName": $("#companyName").val(),
"dangerousChemicalsPlan.planId": $("#planId").val(),
"dangerousChemicalsPlan.dangerLevel": $("#dangerLevel").val(),
"dangerousChemicalsPlan.dangerType": $("#dangerType").val(),
...
"queryTbrqStart" :$("#queryTbrqStart").val(),
 "queryTbrqEnd" :$("#queryTbrqEnd").val()
        ...
        $(function(){
			...
			$('#pagination').datagrid({
				title:'危险化学品从业单位应急救援预案备案事项通知书列表',
				iconCls:'icon-save',
				nowrap: false,
				striped: true,
				collapsible:true,
				url:'dangerousChemicalsPlanQuery.action',
				queryParams:{
				"dangerousChemicalsPlan.szzid": $("#szzid").val(),
					"dangerousChemicalsPlan.companyName": $("#companyName").val(),
"dangerousChemicalsPlan.planId": $("#planId").val(),
...
{field:'dangerLevel',title:'危险源级别',width:fixWidth(0.1)},
{field:'dangerType',title:'危险化学品单位类型',width:fixWidth(0.2)},
{field:'planDate',title:'备案日期',width:fixWidth(0.1)},
{field:'planId',title:'备案编号',width:fixWidth(0.1)},
	...
	<div class="submitdata">
		<table width="100%">
			<tr>
				...
				<td width="35%">
					...
					<cus:SelectOneTag property="dangerousChemicalsPlan.szzid" defaultText='请选择' codeName="相城地址" value="${dangerousChemicalsPlan.szzid}" /></td>
					 -->
				<th width="15%">单位名称</th>
				<td width="35%"><input name="dangerousChemicalsPlan.companyName" id="companyName" value="${dangerousChemicalsPlan.companyName}" type="text"></td>
			</tr>
			<tr>
				<th width="15%">危险源级别</th>
				<td width="35%"><select name="dangerousChemicalsPlan.dangerLevel" id="dangerLevel"><option value="">请选择</option><option value="一级">一级</option><option value="二级">二级</option><option value="三级">三级</option><option value="四级">四级</option></select></td>
			    <th width="15%">危险化学品单位类型</th>
				<td width="35%"><select name="dangerousChemicalsPlan.dangerType" id="dangerType"><option value="">请选择</option><option value="生产">生产</option><option value="使用">使用</option><option value="储存">储存</option><option value="经营">经营</option></select></td>
			</tr>
			<tr>
				<th width="15%">备案日期</th>
			...
			<tr>
				...
				<a href="###" class="easyui-linkbutton" onclick="search_dangerousChemicalsPlan()" iconCls="icon-search">查询</a>&nbsp;
