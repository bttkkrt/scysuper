...
<html>
<head>
<title>挂牌督办隐患整改管理</title>
    <%@include file="/common/jsLib.jsp"%>
	<script>
        var flag = "${flag}";
        var createUserID = "${createUserID}";
        function addNew(){
        	window.open("${ctx}/jsp/gpqyyhzg/gpqyyhzgInitEdit.action?flag=add");
        }
        function edit(row_Id){
        	window.open("${ctx}/jsp/gpqyyhzg/gpqyyhzgInitEdit.action?flag=mod&gpqyyhzg.id="+row_Id);
        }
        function view(row_Id){
       		window.open("${ctx}/jsp/gpqyyhzg/gpqyyhzgView.action?gpqyyhzg.id="+row_Id);
        }
        function exportdata()
        {
        	document.myform.action = "${ctx}/jsp/gpqyyhzg/gpqyyhzgExport.action";
        ...
        function search_gpqyyhzg(){
        	var queryParams = {
				"gpqyyhzg.szzid": $("#szzid").val(),
 "queryTjyfStart" :$("#queryTjyfStart").val(),
 "queryTjyfEnd" :$("#queryTjyfEnd").val()
			};        	
 ...
 "queryTjyfEnd" :$("#queryTjyfEnd").val()
				...
				columns:[[
				          {field:'szzname',title:'所在镇',width:fixWidth(0.2)},
{field:'tjyf',title:'统计月份',width:fixWidth(0.2)},
		...
		<table width="100%">
			...
			<s:if test="flag == 0">
				...
				<td width="35%">
					...
					<select id="szzid" name="gpqyyhzg.szzid" style="width:150px;" ><option value="">请选择</option></select>
						
					<!--hanxc 2014/11/11 		
					<cus:SelectOneTag property="gpqyyhzg.szzid" defaultText='请选择' codeName="相城地址" value="${gpqyyhzg.szzid}" /></td>
					 -->
			</s:if>
				<th width="15%">统计月份</th>
				<td width="35%"><input name="queryTjyfStart" id="queryTjyfStart" value="${queryTjyfStart}" type="text" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM',maxDate:'#F{$dp.$D(\'queryTjyfEnd\')}'})" >
					-<input name="queryTjyfEnd" id="queryTjyfEnd" value="${queryTjyfEnd}" type="text" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM',minDate:'#F{$dp.$D(\'queryTjyfStart\')}'})" ></td>
			</tr>
			<tr>
			</tr>
			<tr>
				...
				<a href="###" class="easyui-linkbutton" onclick="search_gpqyyhzg()" iconCls="icon-search">查询</a>&nbsp;
