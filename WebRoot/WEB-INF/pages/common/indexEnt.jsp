...
<header class="Hui-header cl">
	<div style="width:100%;height:100%;">
		...
		<a class="Hui-logo l" title="回到首页" href="${ctx}/index.action">智慧安监</a>   <span class="Hui-subtitle l"> 云管理平台 </span>
...
<aside class="Hui-aside">
	...
	<div class="menu_dropdown bk_2">
        <div class="menu"  >
            <ul style="margin-bottom: 5px;" id="indexUl">
 				<c:forEach items="${items}" var="moduleitem" varStatus="status1">
						<c:if test="${fn:length(moduleitem.children)>0}">
							<li class='menus'>
								<a href='javascript:void(0)' data-title='${moduleitem.text}' ><icon class=""></icon>${moduleitem.text} <i class='Hui-iconfont Hui-iconfont-arrow2-right'></i></a>
								<ul >
									<c:forEach items="${moduleitem.children}" var="child" varStatus="status2">
										<c:if test="${fn:length(child.children)>0}">
											<li class='menus'>
												<a href='javascript:void(0)' style="    background-color: aliceblue;" data-title='${child.text}' >${child.text} <i class='Hui-iconfont Hui-iconfont-arrow2-right'></i></a>
												<ul>
																		...
																		<c:forEach items="${child2.children}" var="child3" varStatus="status4">
																			<li class='item'>
																				...
																				<c:if test="${not empty child3.attributes}">
																					<a href='javascript:void(0)'  onclick="Hui_admin_tab(this)"	_href="${ctx}/${child3.attributes}?entBaseInfoId=${entBaseInfoId}" data-title='${child3.text}' >${child3.text}</a>
																				</c:if>
																			</li>
																		</c:forEach>
																	</ul>
															...
															<c:if test="${fn:length(child2.children)==0}">
																<li class='item'>
																	...
																	<c:if test="${not empty child2.attributes}">
																		<a href='javascript:void(0)'  onclick="Hui_admin_tab(this)"	_href="${ctx}/${child2.attributes}?entBaseInfoId=${entBaseInfoId}" data-title='${child2.text}' >${child2.text}</a>
																	</c:if>
																</li>
															</c:if>
														</c:forEach>
											...
											<c:if test="${fn:length(child.children)==0}">
												<li class='item'>
													<c:if test="${empty child.attributes}">
														<a href='javascript:void(0)' style="background-color: aliceblue;" onclick="Hui_admin_tab(this)"	_href="${ctx}/personal/error_developing.html" data-title='${child.text}' >${child.text}</a>
													</c:if>
													<c:if test="${not empty child.attributes}">
														<a href='javascript:void(0)' style="background-color: aliceblue;" onclick="Hui_admin_tab(this)"	_href="${ctx}/${child.attributes}?entBaseInfoId=${entBaseInfoId}" data-title='${child.text}' >${child.text}</a>
													</c:if>
												</li>
											</c:if>
									</c:forEach>
						...
						<c:if test="${fn:length(moduleitem.children)==0}">
							<li class='item'>
								<a href='javascript:void(0)'  onclick="Hui_admin_tab(this)" _href="${ctx}/${moduleitem.attributes}" data-title='${moduleitem.text}' ><icon class=""></icon>${moduleitem.text} <i></i></a>
							</li>
						</c:if>
				</c:forEach>
	...
	<div id="Hui-tabNav" class="Hui-tabNav">
		<div class="Hui-tabNav-wp">
			<ul id="min_title_list" class="acrossTab cl">
				<%-- <li class="active"><span title="我的桌面" data-href="${ctx}/jsp/information/contentInformationsList.action?moduleNames=最新公告">企业基本信息</span><em></em></li> --%>
			</ul>
		</div>
...
$(function(){
    $(".menu ul li").menu();
    $("#indexUl li:nth-child(1)").children("a").first().click();
	$("#indexUl li:nth-child(1)").children("ul").children("li").children("a").first().click();
	$("#indexUl li:nth-child(1)").children("ul").children("li").children("ul").children("li").children("a").first().click();
    $('#tabdiv').tabs({
