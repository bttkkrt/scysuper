...
<html>
<head>
<title>检修作业规程管理</title>
    <%@include file="/common/jsLib.jsp"%>
	<script>
        
        function addNew(){
        	window.open("${ctx}/jsp/maintenp/maintenpInitEdit.action?flag=add");	
        }
        function edit(row_Id){
        	window.open("${ctx}/jsp/maintenp/maintenpInitEdit.action?flag=mod&maintenp.id="+row_Id);	
        }
        function view(row_Id){
        	 window.open("${ctx}/jsp/maintenp/maintenpView.action?maintenp.id="+row_Id);	
        }
        function close_win(){
        ...
        function search_maintenp(){
        	var queryParams = {
				"maintenp.producename": $("#producename").val(),
					"maintenp.deptId":$("#deptId").val(),
					"maintenp.comName":$("#comName").val(),
				...
				columns:[[
				 {field:'deptName',title:'所属街道',width:fixWidth(0.15)},
					 {field:'comName',title:'企业名称',width:fixWidth(0.2)},
				          {field:'producename',title:'制度名称',width:fixWidth(0.25),formatter:function(value,rec){
  var temp = '';
    $.ajax({
    ...
    data:{        "codeValue.itemValue" : rec.producename,
        "codeValue.codeId" : "4028e56c407acbd001407ae4e456001d"    },
    error: function(){
...
<form name="myform" method="post">
	<div class="submitdata">
		<table width="100%">
			<tr>
				<th width="15%">企业名称</th>
				<td width="35%"><input name="maintenp.comName" size=30 id="comName" value="${maintenp.comName}" type="text"></td>
				<th width="15%">规程名称</th>
				<td width="35%"><cus:SelectOneTag property="maintenp.producename" defaultText='请选择' codeName="检修作业规程" value="${maintenp.producename}" /></td>
			</tr>
			<tr>
				...
				<s:if test="visable==1">
					...
					<select id="deptId" name="maintenp.deptId" style="width:150px;" ><option value="">请选择</option></select>
						
					<!--hanxc 2014/11/11 		
					<cus:SelectOneTag property="maintenp.deptId" defaultText='请选择' codeName="相城地址" value="${maintenp.deptId}" /></td>
					 -->
				</s:if>
			</tr>
			<tr>
				<td colspan="6" style="text-align:center">
				<a href="###" class="easyui-linkbutton" onclick="search_maintenp()" iconCls="icon-search">查询</a>&nbsp;
