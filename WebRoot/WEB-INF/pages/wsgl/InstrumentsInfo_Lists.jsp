...
<title>文书管理管理</title>
	...
	<script>
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_instrumentsInfo","添加文书","${ctx}/jsp/wsgl/instrumentsInfoInitEdit.action?flag=add&dt="+dt.getTime()+"&instrumentsInfo.caseId=${instrumentsInfo.caseId}",900,550);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_instrumentsInfo","修改文书","${ctx}/jsp/wsgl/instrumentsInfoInitEdit.action?flag=mod&instrumentsInfo.id="+row_Id+"&dt="+dt.getTime(),900,550);
        	...
        	if(type == '0')
        	{
				...
				frozenColumns = [[
				    {field:'id',width:30,formatter:function(value,rec){
						    ...
						    if(rec.createUserID != loginUserId || rec.ifCheck != 4){//这里判断创建人是不是登录人
								opt = ''; 
						    } 
						    return opt ; 
