...
<head>
	...
	<script>
		 ...
		 $(document).ready(function(){
			    //获取组织机构人员
			    var userIds=window.opener.document.getElementById("attendId").value;
			    var userNames=window.opener.document.getElementById("attendName").value;
                var u=userNames.substring(0,userNames.length-1); 
                var ids=userIds.split(",");
                var names=u.split(",");
			    var t=new Date().getTime();
			    $.post('${ctx}/jsp/wsgl/getDepartUser.action',{'t':t,'userIds':userIds},function(nodes){
 ...
 function returnCheckedUsers(flowType) {
		...
		if(userids&&""!=userids){
			window.opener.document.getElementById("attendId").value = userids.substring(0,userids.length-1);
			window.opener.document.getElementById("attendName").value = usernames.substring(0,usernames.length-1);
			window.close();			
		}else{
			alert("请选择出席人员");
		}
