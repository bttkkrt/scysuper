...
<html>
<head>
<title>资格证书扫描件管理</title>
    <%@include file="/common/jsLib.jsp"%>
	<script>
         var flag = "${flag}";
        function addNew(){
        	window.open("${ctx}/jsp/zsgl/jshxZsglInitEdit.action?flag=add");
        }
        function edit(row_Id){
            window.open("${ctx}/jsp/zsgl/jshxZsglInitEdit.action?flag=mod&jshxZsgl.id="+row_Id);
        }
        function view(row_Id){
        	window.open("${ctx}/jsp/zsgl/jshxZsglView.action?jshxZsgl.id="+row_Id);
        }
        function close_win(){
        ...
        function search_jshxZsgl(){
        	var queryParams = {
        		"jshxZsgl.szzid": $("#szzid").val(),
        		"jshxZsgl.qymc": $("#qymc").val(),
				"jshxZsgl.linkman": $("#linkman").val(),
				"jshxZsgl.jshxType": $("#jshxType").val(),
			 ...
			 "createTimeEnd" :$("#createTimeEnd").val()
				...
				{field:'szzname',title:'所属地区',width:fixWidth(0.15)},
						{field:'qymc',title:'企业名称',width:fixWidth(0.15)},
				          {field:'linkman',title:'证书持有人姓名',width:fixWidth(0.15)},
							{field:'jshxType',title:'证书类型',width:fixWidth(0.15),formatter:function(value,rec){
							  var temp = '';
							    $.ajax({
							    ...
							    dataType: 'json',
							    async : false,
							    data:{        "codeValue.itemValue" : rec.jshxType,
							        "codeValue.codeId" : "4028e572407b3ecd01407be8a37e009b"},
							    error: function(){
...
<form name="myform" method="post">
	<div class="submitdata">
		<table width="100%">
			<tr>
				<th width="15%">企业名称</th>
				<td width="35%"><input name="jshxZsgl.qymc" id="qymc" value="${jshxZsgl.qymc}" type="text"></td>
				<th width="15%">证书持有人姓名</th>
				<td width="35%"><input name="jshxZsgl.linkman" id="linkman" value="${jshxZsgl.linkman}" type="text"></td>
			</tr>
			<tr>
				<th width="15%">证书类型</th>
				<td width="35%"><cus:SelectOneTag property="jshxZsgl.jshxType" defaultText='请选择' codeName="证书类型" value="${jshxZsgl.jshxType}" /></td>
				...
				<s:if test="flag==3">
					...
					<select id="szzid" name="jshxZsgl.szzid" style="width:150px;" ><option value="">请选择</option></select>
						
					<!--hanxc 2014/11/11 		
					<cus:SelectOneTag property="jshxZsgl.szzid" defaultText='请选择' codeName="相城地址" value="${jshxZsgl.szzid}" /></td>
					 -->
					</tr>
				</s:if>
			<tr>
			...
			<tr>
				...
				<a href="###" class="easyui-linkbutton" onclick="search_jshxZsgl()" iconCls="icon-search">查询</a>&nbsp;
