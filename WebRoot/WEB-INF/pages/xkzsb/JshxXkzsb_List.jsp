...
<html>
<head>
<title>行政许可证书文件管理</title>
    <%@include file="/common/jsLib.jsp"%>
    <script src=”http://html5shiv.googlecode.com/svn/trunk/html5.js”></script>
	<script>
         var flag = "${flag}";
        function addNew(){
        	window.open("${ctx}/jsp/xkzsb/jshxXkzsbInitEdit.action?flag=add");
        }
        function edit(row_Id){
        	window.open("${ctx}/jsp/xkzsb/jshxXkzsbInitEdit.action?flag=mod&jshxXkzsb.id="+row_Id);
        }
        function view(row_Id){
        	window.open("${ctx}/jsp/xkzsb/jshxXkzsbView.action?jshxXkzsb.id="+row_Id);
        }
        function close_win(){
        ...
        function search_jshxXkzsb(){
        	var queryParams = {
        	"jshxXkzsb.szzid": $("#szzid").val(),
				"jshxXkzsb.qymc": $("#qymc").val(),
				"jshxXkzsb.jshxType": $("#jshxType").val(),
			 "createTimeStart" :$("#createTimeStart").val(),
			 "createTimeEnd" :$("#createTimeEnd").val(),
			 "jshxXkzsb.qyid": $("#qyid").val()
			};        	
			 ...
			 //"jshxXkzsb.qyid": $("#qyid").val()
				...
				columns:[[
						...
						{field:'qymc',title:'企业名称',width:fixWidth(0.2)},
				          {field:'jshxType',title:'许可证类型',width:fixWidth(0.2),formatter:function(value,rec){
							  var temp = '';
							    $.ajax({
							    ...
							    data:{        "codeValue.itemValue" : rec.jshxType,
							        "codeValue.codeId" : "4028804840e1b0300140e77daec20308"    },
							    error: function(){
						 ...
						 {field:'createTime',title:'上传时间',width:fixWidth(0.2),formatter:function(value,rec){
				          ...
				          {field:'op',title:'查看',width:fixWidth(0.15),formatter:function(value,rec){
	                           /** var str="";
								  $.ajax({
...
<form name="myform" method="post">
	<input type="hidden" value="${jshxXkzsb.qyid}" id="qyid"/>
	<div class="submitdata">
		<table width="100%">
			
			<tr>
				<th width="15%">许可证类型</th>
				<td width="35%"><cus:SelectOneTag property="jshxXkzsb.jshxType" defaultText='请选择' codeName="行政许可" value="${jshxXkzsb.jshxType}" /></td>
			...
			<tr>
				<s:if test="flag==3">
					...
					<td width="35%">
						...
						<select id="szzid" name="jshxXkzsb.szzid" style="width:100px;" ><option value="">请选择</option></select>
						<!--hanxc 2014/11/8 
						<cus:SelectOneTag property="jshxXkzsb.szzid" defaultText='请选择' codeName="相城地址" value="${jshxXkzsb.szzid}" />
						
						 -->
					</td>
				</s:if>
				<th width="15%">企业名称</th>
				<td width="35%"><input name="jshxXkzsb.qymc" id="qymc" value="${jshxXkzsb.qymc}" type="text"></td>
			</tr>
			</s:if>
			<tr>
				<td colspan="4" align="center" style="text-align:center;">
				<a href="###" class="easyui-linkbutton" onclick="search_jshxXkzsb()" iconCls="icon-search">查询</a>&nbsp;
