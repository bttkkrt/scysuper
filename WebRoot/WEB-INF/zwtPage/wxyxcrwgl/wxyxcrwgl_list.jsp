    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、任务名称或巡查人员姓名"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、任务名称或巡查人员姓名");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		+item.companyName+"</td><td>"+item.taskName+"</td><td>"
		                    		+item.checkTime.substring(0,10)+"</td><td>"+item.checkTimeEnd.substring(0,10)+"</td><td>"
		                     		+item.checkPeopleName+"</td><td>"+item.result+"</td>"
        ...
        function getButton(rec){
	        var temp =  "<a style='color:blue' href='#' onclick=view('"+rec.id+"') >查看<b></b></a>&nbsp;";
	
				  if(rec.result=='已巡查' ){
				 	 	//temp += "<a style='color:blue' href='#' onclick=taskYhbList('"+rec.id+"')>隐患信息<b></b></a>&nbsp;";
				 	}
				 	if(rec.result=='待巡查' && '${roleName}'=='0' && rec.assPlanNo == ''){
				 		temp += "<a style='color:blue' href='#' onclick=edit('"+rec.id+"')>修改<b></b></a>";
					 }
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_danTasMan","添加巡查任务","${ctx}/zwt/danTasManInitEdit.action?flag=add&dt="+dt.getTime(),700,300);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_danTasMan","修改巡查任务","${ctx}/zwt/danTasManInitEdit.action?flag=mod&danTasMan.id="+row_Id+"&dt="+dt.getTime(),700,300);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_danTasMan","查看巡查任务详情","${ctx}/zwt/danTasManView.action?danTasMan.id="+row_Id+"&dt="+dt.getTime(),700,350);
        ...
        function taskYhbList(row_Id){
	        ...
	        window.parent.addTab("win_yhbList","查看隐患列表",url);
        }
...
<!-- head style -->
    	<div class="sipac_head">
        	<a href="#" class="s_h_logo"><i></i><span>⑧隐患排查和治理</span></a>
            <div class="s_h_nav">
                <ul>
                	<li>
                		<a href="${ctx }/zwt/danPlaManList.action">危险源巡查计划管理</a>
                  </li>
                  <li>
                		<a class="active" href="${ctx }/zwt/danTasManList.action">危险源巡查任务管理</a>
                  </li>
                  <li>
                		<a href="${ctx }/zwt/qyzcyhListZwt.action">企业自查隐患</a>
                  </li>
            ...
            <div class="sipac_rtbox">
            	...
            	<div class="sipac_searchbar">
                	<input class="s_s_text" style='width:230px;' type="text" id="searchLike" value="搜索企业名称、任务名称或巡查人员姓名"><input class="s_s_btn" type="submit" onclick="searchByNameOrCode();"><!-- 
                	...
                	<table>
                        ...
                        <tr>
                        	...
                        	<th><span>任务名称 </span></th>
                				  <th><span>巡查开始时间 </span></th>
                          <th><span>巡查结束时间 </span></th>  
                          <th><span>巡查人员姓名 </span></th>  
                          <th><span>巡查结果 </span></th>                                   	
                        	<th ><span>操作项</span></th>
                        </tr>
  						</thead>
                    	<tbody id="tbody">
