    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、车间名称或岗位名称"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、车间名称或岗位名称");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		+item.operationWorkshopName+"</td><td>"+item.operationPostname+"</td>"
      ...
      function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_opePro","添加操作规程","${ctx}/zwt/opeProInitEdit.action?flag=add&dt="+dt.getTime(),700,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_opePro","修改操作规程","${ctx}/zwt/opeProInitEdit.action?flag=mod&opePro.id="+row_Id+"&dt="+dt.getTime(),700,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_opePro","查看操作规程","${ctx}/zwt/opeProView.action?opePro.id="+row_Id+"&dt="+dt.getTime(),700,500);
...
<!-- head style -->
    	<div class="sipac_head">
            ...
            <div class="s_h_nav">
                <ul>
                	<li>
                		<a href="${ctx }/zwt/legStaList.action">法律法规标准规范</a>
                  </li>
                  <li>
                		<a href="${ctx }/zwt/secProSysList.action">安全生产规章制度</a>
                  </li>
                  <li>
                		<a class="active" href="${ctx }/zwt/opeProList.action">操作规程</a>
                  </li>
            ...
            <div class="sipac_rtbox">
            	...
            	<div class="sipac_searchbar">
                	<input class="s_s_text" style='width:250px;' type="text" id="searchLike" value="搜索企业名称、车间名称或岗位名称"><input class="s_s_btn" type="submit" onclick="searchByNameOrCode();"><!-- 
