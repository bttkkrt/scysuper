    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、计划名称或巡查人员姓名"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、计划名称或巡查人员姓名");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		+item.planName+"</td><td>"+item.checkFrequency+"</td><td>"
		                    		+item.checkPeopleName+"</td><td>"+isAudit+"</td><td>"
		                     		+auditResult+"</td>"
        ...
        function getButton(rec){
	        var temp =  "<a style='color:blue' href='#' onclick=view('"+rec.id+"') >查看<b></b></a>&nbsp;";
					...
					if(rec.auditResult == '审核通过'){
						 //temp += "<a style='color:blue' href='#' onclick=taskList('"+rec.id+"')>查看任务<b></b></a>&nbsp;";
					}
					//监察大队 执行审核操作
					if(rec.auditResult == '待审核'&&'${roleName}' == '1'){
						temp += "<a style='color:blue' href='#' onclick=audit('"+rec.id+"')>审核<b></b></a>";
					}
					return temp;
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_danPlaMan","添加巡查计划","${ctx}/zwt/danPlaManInitEdit.action?flag=add&dt="+dt.getTime(),700,300);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_danPlaMan","修改巡查计划","${ctx}/zwt/danPlaManInitEdit.action?flag=mod&danPlaMan.id="+row_Id+"&dt="+dt.getTime(),700,300);
...
<!-- head style -->
    	<div class="sipac_head">
            ...
            <div class="s_h_nav">
                <ul>
                	<li>
                		<a class="active" href="${ctx }/zwt/danPlaManList.action">危险源巡查计划管理</a>
                  </li>
                  <li>
                		<a href="${ctx }/zwt/danTasManList.action">危险源巡查任务管理</a>
                  </li>
                  <li>
            ...
            <div class="sipac_rtbox">
            	...
            	<div class="sipac_searchbar">
                	<input class="s_s_text" style='width:230px;' type="text" id="searchLike" value="搜索企业名称、计划名称或巡查人员姓名"><input class="s_s_btn" type="submit" onclick="searchByNameOrCode();"><!-- 
