...
<script src="${ctx}/webResources/zwt/layer/layer.js"></script>
	<script>
	  function Sbcs(dwdz,flags){
	  	  var dt=new Date();
	  	  createSimpleWindow("win_secProIns","查看隐患情况","${ctx}/zwt/aqscyhView.action?troMan.areaId="+dwdz+"&flag=0"+ flags + "&dt="+dt.getTime()+"&starttime=${starttime}&endtime=${endtime}",1100,500);
	  }
	  
	  function Sbqy(dwdz,flags){
	  	  var dt=new Date();
	  	  createSimpleWindow("win_secProIns","查看上报企业情况","${ctx}/zwt/yhcomView.action?troMan.areaId="+dwdz+"&flag=00"+ flags + "&dt="+dt.getTime()+"&starttime=${starttime}&endtime=${endtime}",1100,500);
	  }
	  
	  function Wsbqy(dwdz,flags){
	  	  var dt=new Date();
	  	  createSimpleWindow("win_secProIns","查看未上报企业情况","${ctx}/zwt/yhcomView.action?troMan.areaId="+dwdz+"&flag=01"+ flags + "&dt="+dt.getTime()+"&starttime=${starttime}&endtime=${endtime}",1100,500);
	  }
