...
<title>挂牌督办管理</title>
	...
	<script>
        ...
        function check1(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supervice","审核挂牌督办","${ctx}/jsp/gpdb/superviceCheck.action?flag=check1&supervice.id="+row_Id+"&dt="+dt.getTime(),900,500);
        }
        function check2(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supervice","审批挂牌督办","${ctx}/jsp/gpdb/superviceCheck.action?flag=check2&supervice.id="+row_Id+"&dt="+dt.getTime(),900,500);
        }
        ...
        function report(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supervice","上报整改信息","${ctx}/jsp/gpdb/superviceInitEdit.action?flag=report&supervice.id="+row_Id+"&dt="+dt.getTime(),900,500);
...
{field:'op',title:'操作',width:100,formatter:function(value,rec){
	...
	var button5="<a class='btn_02_mini' onclick=view('"+rec.id+"') >查看<b></b></a>&nbsp;<a class='btn_03_mini' onclick=report('"+rec.id+"')>整改上报<b></b></a>";
    ...
    <div class="page_content">
		...
		<div class="inner12px">
			...
			<tr>
				...
				<th width="15%">整改状态</th>
				<td width="35%">
					<s:select id="rectificationState"  name="supervice.rectificationState" cssStyle="width:50%;"  list="#{'':'请选择','待修改':'待修改','待审核':'待审核','待审批':'待审批','待整改':'待整改','已整改待审核':'已整改待审核','已整改待审批':'已整改待审批','完成':'完成'}" theme="simple"/>
				</td>
			</tr>
			<tr>
				</tr>
