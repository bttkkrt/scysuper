    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索计划名称或工作目标"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索计划名称或工作目标");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		var workGoal=item.workGoal;
		                    		if(planName.length>10){
		                    			planName=planName.substring(0,10)+"...";
		                    		}
		                    		if(workGoal.length>10){
		                    			workGoal=workGoal.substring(0,10)+"...";
		                    		}
		                    		var tr="";
		                    		if(idx%2==0){
		                    		...
		                    		+workGoal+"</td><td>"+item.workTimeStart.substring(0,11)+"</td><td>"
		                    		+item.workTimeEnd.substring(0,11)+"</td>"
        ...
        function getButton(rec){
			...
			var button2="<a style='color:blue' href='#' onclick=view('"+rec.id+"') >查看<b></b></a>&nbsp;<a style='color:blue' href='#' onclick=viewCheck('"+rec.id+"')>检查记录<b></b></a>";
			if(rec.status=='0'){
				return button1;
			}else{
				return button2;
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_recPla","添加专项整治计划","${ctx}/zwt/recPlaInitEdit.action?flag=add&dt="+dt.getTime(),700,550);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_recPla","修改专项整治计划","${ctx}/zwt/recPlaInitEdit.action?flag=mod&recPla.id="+row_Id+"&dt="+dt.getTime(),700,550);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_recPla","查看专项整治计划","${ctx}/zwt/recPlaView.action?recPla.id="+row_Id+"&dt="+dt.getTime(),700,550);
        	
        }
        function viewCheck(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_recRes","查看专项整治检查记录","${ctx}/zwt/recResViewByPlan.action?recPla.id="+row_Id+"&dt="+dt.getTime(),700,550);
...
<!-- head style -->
    	<div class="sipac_head">
            ...
            <div class="s_h_nav">
                <ul>
                	<li>
                		<a class="active" href="${ctx }/zwt/recPlaList.action">专项整治计划管理</a>
                    </li>
                    <li>
            ...
            <div class="sipac_rtbox">
            	...
            	<div class="sipac_searchbar">
                	<input class="s_s_text" type="text" id="searchLike" value="搜索计划名称或工作目标"><input class="s_s_btn" type="submit" onclick="searchByNameOrCode();"><!-- 
            	...
            	<div class="sipac_tableBox">
                	...
                	<table>
                        ...
                        <tr>
                        	...
                        	<th><span>工作目标 </span></th>
                        	<th><span>整治开始时间   </span></th>
                        	<th><span>整治结束时间 </span></th>
                        	<th ><span>操作项</span></th>
                        </tr>
  						</thead>
                    	<tbody id="tbody">
