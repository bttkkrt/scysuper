    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、机构名称或负责人"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、机构名称或负责人");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		var orgenizationName=item.orgenizationName;
		                    		if(orgenizationName.length>10){
		                    			orgenizationName=orgenizationName.substring(0,10)+"...";
		                    		}
		                    		
		                    		var orgenizationResponsibility=item.orgenizationResponsibility;
		                    		if(orgenizationResponsibility.length>10){
		                    			orgenizationResponsibility=orgenizationResponsibility.substring(0,10)+"...";
		                    		}
		                    		...
		                    		tr+="<td>"+item.areaId+"</td><td>"
		                    		+item.companyName+"</td><td>"
		                    		+orgenizationName+"</td><td>"+orgenizationResponsibility+"</td><td>"
		                    		+item.orgenizationMenberCount+"</td><td>"+item.orgenizationCharge+"</td>"
        ...
        function getButton(rec){
        	if('${roleName}'=='0'){
						...
						return "<a style='color:blue' href='#' onclick=view('"+rec.id+"') >查看<b></b></a>";
					}
         ...
         function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_proManOrg","添加安全生产管理机构","${ctx}/zwt/proManOrgInitEdit.action?flag=add&dt="+dt.getTime(),800,300);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proManOrg","修改安全生产管理机构","${ctx}/zwt/proManOrgInitEdit.action?flag=mod&proManOrg.id="+row_Id+"&dt="+dt.getTime(),800,300);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proManOrg","查看安全生产管理机构","${ctx}/zwt/proManOrgView.action?proManOrg.id="+row_Id+"&dt="+dt.getTime(),800,350); 
...
<!-- head style -->
    	<div class="sipac_head">
        	<a href="#" class="s_h_logo"><i></i><span>②组织机构职责</span></a>
            <div class="s_h_nav">
                <ul>
                	<li>
                		<a href="${ctx }/zwt/proLegOrgList.action">领导机构</a>
                    </li>
                	<li>
                		<a class="active" href="${ctx }/zwt/proManOrgList.action">管理机构</a>
                    </li>
                    <li>
                		<a href="${ctx }/zwt/entChaPerList.action">主要负责人</a>
                    </li>
                    <li>
                		<a href="${ctx }/zwt/secProChaList.action">分管负责人</a>
                    </li>
                     <li>
                		<a href="${ctx }/zwt/secProManList.action">管理经理</a>
                    </li>
                     <li>
                		<a href="${ctx }/zwt/secProMajList.action">生产总监</a>
                    </li>
                     <li>
                		<a href="${ctx }/zwt/secProChaPerList.action">负责人员</a>
                    </li>
                     <li>
                		<a href="${ctx }/zwt/speJobPerList.action">特种作业人员</a>
                    </li>
                </ul>
            </div>
            ...
            <div class="sipac_rtbox">
            	...
            	<div class="sipac_searchbar">
                	<input class="s_s_text" style='width:250px;' type="text" id="searchLike" value="搜索企业名称、机构名称或负责人"><input class="s_s_btn" type="submit" onclick="searchByNameOrCode();"><!-- 
