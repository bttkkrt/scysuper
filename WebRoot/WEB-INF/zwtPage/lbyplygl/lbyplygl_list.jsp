    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、用品名称或领用人"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、用品名称或领用人");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		+item.companyName+"</td><td>"+item.ppeName+"</td><td>"
		                    		+item.ppeUsePeople+"</td><td>"
		                     		+item.ppeUseNum+"</td>"
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_ppeUseManage","添加劳保用品领用管理","${ctx}/zwt/ppeUseManageInitEdit.action?flag=add&dt="+dt.getTime(),700,300);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_ppeUseManage","修改劳保用品领用管理","${ctx}/zwt/ppeUseManageInitEdit.action?flag=mod&ppeUseManage.id="+row_Id+"&dt="+dt.getTime(),700,300);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_ppeUseManage","查看劳保用品领用管理","${ctx}/zwt/ppeUseManageView.action?ppeUseManage.id="+row_Id+"&dt="+dt.getTime(),700,300);
        	
        }
        function importUser(){ 
        	createSimpleWindow("importUser","批量导入劳保用品领用","${ctx}/zwt/initImportPpeUseManage.action", 350, 150);
        }
...
<!-- head style -->
    	<div class="sipac_head">
        	<a href="#" class="s_h_logo"><i></i><span>⑦劳保用品配置</span></a>
            <div class="s_h_nav">
                <ul>
                	<li>
                		<a href="${ctx }/zwt/ppeManagList.action">劳保用品管理</a>
                  </li>
                  <li>
                		<a href="${ctx }/zwt/ppeWareManagList.action">劳保用品库房管理施</a>
                  </li>
                  <li>
                		<a class="active" href="${ctx }/zwt/ppeUseManageList.action">劳保用品领用管理</a>
                  </li>
            ...
            <div class="sipac_rtbox">
            	...
            	<div class="sipac_searchbar">
                	<input class="s_s_text" style='width:200px;' type="text" id="searchLike" value="搜索企业名称、用品名称或领用人"><input class="s_s_btn" type="submit" onclick="searchByNameOrCode();"><!-- 
            	...
            	<div class="sipac_tableBox">
                	...
                	<table>
                        ...
                        <tr>
                        	...
                        	<th><span>用品名称 </span></th>
                				  <th><span>领用人 </span></th>
                          <th><span>领用数量 </span></th>  
