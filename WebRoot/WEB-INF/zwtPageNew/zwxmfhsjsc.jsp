...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.industryCategory)+"</a></td><td>"
		                    		+getSpanByName(item.areaName)+"</td><td>"+getSpanByName(item.companyName)+"</td><td>"+getSpanByName(item.reviewDate.substring(0,10))+"</td><td>"+getSpanByName(item.review)+"</td>";
		                    		if('${roleName}'=='1')
		                    		{
        ...
        function getButton(rec){
         ...
         function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_occConDes","添加职业病危害建设项目防护设计审查","${ctx}/zwt/occConDesInitEdit.action?flag=add&dt="+dt.getTime(),900,400);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_occConDes","修改职业病危害建设项目防护设计审查","${ctx}/zwt/occConDesInitEdit.action?flag=mod&occConDes.id="+row_Id+"&dt="+dt.getTime(),900,400);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_occConDes","查看职业病危害建设项目防护设计审查","${ctx}/zwt/occConDesView.action?occConDes.id="+row_Id+"&dt="+dt.getTime(),900,400);
            	...
            	<div class="sipac_tableBox">
                	...
                	<table style="width:100%;">
                        ...
                        <tr>
                        	...
                        	<th><span>审查日期 </span></th>
                        	<th><span>审查结果 </span></th>
                        	<th ><span>操作项</span></th>
                        </tr>
  						</thead>
                    	</table>
