...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.licenseNumber)+"</a></td>"
		                    		+"<td>"+getSpanByName(item.areaId)+"</td>"
		                    		+ "<td>"+getSpanByName(item.companyName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.fileNo)+"</td>"
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_preProLic","添加非药品类易制毒化学品生产许可证","${ctx}/zwt/preProLicInitEdit.action?flag=add&dt="+dt.getTime(),900,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_preProLic","修改非药品类易制毒化学品生产许可证","${ctx}/zwt/preProLicInitEdit.action?flag=mod&preProLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_preProLic","查看非药品类易制毒化学品生产许可证","${ctx}/zwt/preProLicView.action?preProLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
        	
        }
         function check(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_preProLic","审核非药品类易制毒化学品生产许可证","${ctx}/zwt/preProLicCheck.action?flag=check&preProLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
