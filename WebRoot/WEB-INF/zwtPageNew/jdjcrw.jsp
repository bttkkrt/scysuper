...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.taskNum)+"</a></td><td>"
		                    		+getSpanByName(item.companyName)+"</td><td>"
		                    		+getSpanByName(item.stime.substring(0,10))+"</td><td>"+getSpanByName(item.ftime.substring(0,10))+"</td><td>"
		                    		+getSpanByName(item.checkUsername)+"</td><td>"+getSpanByName(item.taskState)+"</td><td>"
		                    		+getSpanByName(item.taskType)+"</td>"
        ...
        function getButton(rec){
			...
			var button1="<a class='s_t_Btn stb_1' href='javascript:;' onclick=sbxx('"+rec.id+"')>上报<b></b></a>&nbsp;";
			var button2="<a class='s_t_Btn stb_1' href='javascript:;' onclick=edit('"+rec.id+"')>修改<b></b></a>&nbsp;";
			var button4="<a class='s_t_Btn stb_3' href='javascript:;' onclick=createXcjl('"+rec.id+"') >检查记录<b></b></a>";
			var button = "";
			if(rec.taskState=="未完成"&&("${sessionScope['LOGIN_USER_ID']}"==rec.checkUserid || "${sessionScope['LOGIN_USER_ID']}"==rec.xbUserId)){
				button +=  button1;
			}
			if(rec.taskState=="未完成"&&(rec.taskType=="临时任务")&&("${sessionScope['LOGIN_USER_ID']}"==rec.createUserID)){
				button += button2;
			}
			if(("${sessionScope['LOGIN_USER_ID']}"==rec.checkUserid || "${sessionScope['LOGIN_USER_ID']}"==rec.xbUserId)&&rec.taskState=="已完成")
			{
				button += button4;
			}
			return button;
         ...
         function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_supTas","添加临时任务","${ctx}/zwt/supTasInitEdit.action?flag=add&dt="+dt.getTime(),900,550);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supTas","修改监督检查任务","${ctx}/zwt/supTasInitEdit.action?flag=mod&supTas.id="+row_Id+"&dt="+dt.getTime(),900,550);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supTas","查看监督检查任务","${ctx}/zwt/supTasView.action?supTas.id="+row_Id+"&dt="+dt.getTime(),900,550);
        	
        }
        
        function sbxx(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supTas","上报检查信息","${ctx}/zwt/supTasSbxx.action?supTas.id="+row_Id+"&dt="+dt.getTime(),900,550);
       ...
       {
       		var dt=new Date();
            createSimpleWindow("win_supTas","现场检查记录","${ctx}/zwt/supTasXcjc.action?supTas.id="+row_Id+"&dt="+dt.getTime(),900,550);
       }
        ...
        <div class="sipac_body" >
            <div class="sipac_container" id="tableDiv" style="margin: 0 0 0 0px;">
            	...
            	<div class="sipac_tableBox">
	                <c:if test="${roleName=='09'||roleName=='11'}">
	                    <div class="sipac_tableBar">
	                        <div class="sipac_table_menu"><a href="javascript:addNew();">新建</a><!--<a href="#">批量删除</a><em></em><a href="#">通过</a><em></em><a href="#">禁用</a><em></em><a href="#">启用</a> --> 
	                       </div>
	                    </div>
	                </c:if>
