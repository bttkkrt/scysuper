...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		+getSpanByName(item.safetyName)+"</a></td><td>"+getSpanByName(item.mobile)+"</td><td>"+getSpanByName(item.jobTitle)+"</td><td>"+getSpanByName(item.education)+"</td>"
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_safetyExperts","添加安全生产专家库","${ctx}/zwt/safetyExpertsInitEdit.action?flag=add&dt="+dt.getTime(),700,550);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_safetyExperts","修改安全生产专家库","${ctx}/zwt/safetyExpertsInitEdit.action?flag=mod&safetyExperts.id="+row_Id+"&dt="+dt.getTime(),700,550);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_safetyExperts","查看安全生产专家库","${ctx}/zwt/safetyExpertsView.action?safetyExperts.id="+row_Id+"&dt="+dt.getTime(),700,550);
