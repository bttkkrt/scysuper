...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.designUnit)+"</a></td>"
		                    	    +"<td>"+getSpanByName(item.areaName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.companyName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.projectNature)+"</td>"
		                    		+ "<td>"+getSpanByName(item.reviewDate)+"</td>";
		                    		if('${roleName}'=='1')
		                    		{
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_cheSafDes","添加危险化学品建设项目安全设施设计审查","${ctx}/zwt/cheSafDesInitEdit.action?flag=add&dt="+dt.getTime(),900,450);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheSafDes","修改危险化学品建设项目安全设施设计审查","${ctx}/zwt/cheSafDesInitEdit.action?flag=mod&cheSafDes.id="+row_Id+"&dt="+dt.getTime(),900,450);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheSafDes","查看危险化学品建设项目安全设施设计审查","${ctx}/zwt/cheSafDesView.action?cheSafDes.id="+row_Id+"&dt="+dt.getTime(),900,450);
