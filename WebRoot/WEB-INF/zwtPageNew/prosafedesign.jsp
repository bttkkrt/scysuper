    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称或备案编号"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称或备案编号");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.recordDate)+"</a></td>"
		                    		+ "<td>"+getSpanByName(item.areaName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.companyName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.projectType)+"</td>"
		                    		+ "<td>"+getSpanByName(item.recordNum.substring(0,10))+"</td>"
		                    		+ "<td>"+getSpanByName(item.projectNature)+"</td>";
		                    		if('${roleName}'=='1')
		                    		{
        ...
        function getButton(rec){
			...
			var button2="";
			if('${roleName}'=='1' && "${sessionScope['LOGIN_USER_ID']}"==rec.createUserID){
				return  button1;
			}else{
				return button2;
			}
      ...
      function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_proSafDes","添加建设项目安全设施设计审查","${ctx}/zwt/proSafDesInitEdit.action?flag=add&dt="+dt.getTime(),900,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proSafDes","修改建设项目安全设施设计审查","${ctx}/zwt/proSafDesInitEdit.action?flag=mod&proSafDes.id="+row_Id+"&dt="+dt.getTime(),900,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proSafDes","查看建设项目安全设施设计审查","${ctx}/zwt/proSafDesView.action?proSafDes.id="+row_Id+"&dt="+dt.getTime(),900,500);
