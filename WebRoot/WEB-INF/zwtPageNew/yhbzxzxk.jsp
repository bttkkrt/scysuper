...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.itemNo)+"</a></td>"
		                    		+"<td>"+getSpanByName(item.areaName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.companyName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.licenseValid.substring(0,10))+"</td>"
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_firLic","添加烟花爆竹行政许可","${ctx}/zwt/firLicInitEdit.action?flag=add&dt="+dt.getTime(),1150,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_firLic","修改烟花爆竹行政许可","${ctx}/zwt/firLicInitEdit.action?flag=mod&firLic.id="+row_Id+"&dt="+dt.getTime(),1150,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_firLic","查看烟花爆竹行政许可","${ctx}/zwt/firLicView.action?firLic.id="+row_Id+"&dt="+dt.getTime(),1150,500);
        	
        }
         function check(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_firLic","审核烟花爆竹行政许可","${ctx}/zwt/firLicCheck.action?flag=check&firLic.id="+row_Id+"&dt="+dt.getTime(),1150,500);
