...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.proDate.substring(0,11))+"</a></td><td>"
		                    		+getSpanByName(item.areaName)+"</td><td>"+getSpanByName(item.companyName)+"</td>";
		                    		if('${roleName}'=='1')
		                    		{
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_proComNot","添加建设项目职业病防护设施竣工备案通知书","${ctx}/zwt/proComNotInitEdit.action?flag=add&dt="+dt.getTime(),800,550);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proComNot","修改建设项目职业病防护设施竣工备案通知书","${ctx}/zwt/proComNotInitEdit.action?flag=mod&proComNot.id="+row_Id+"&dt="+dt.getTime(),800,550);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proComNot","查看建设项目职业病防护设施竣工备案通知书","${ctx}/zwt/proComNotView.action?proComNot.id="+row_Id+"&dt="+dt.getTime(),800,550);
