...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.occupationalClassification)+"</a></td><td>"
		                    		+getSpanByName(item.areaName)+"</td><td>"+getSpanByName(item.companyName)+"</td><td>"+getSpanByName(item.acceptanceExpert)+"</td><td>"+getSpanByName(item.industryCategory)+"</td>";
		                    		if('${roleName}'=='1')
		                    		{
        ...
        function getButton(rec){
         ...
         function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_occConCom","添加职业病危害建设项目防护设施竣工审查","${ctx}/zwt/occConComInitEdit.action?flag=add&dt="+dt.getTime(),900,400);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_occConCom","修改职业病危害建设项目防护设施竣工审查","${ctx}/zwt/occConComInitEdit.action?flag=mod&occConCom.id="+row_Id+"&dt="+dt.getTime(),900,400);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_occConCom","查看职业病危害建设项目防护设施竣工审查","${ctx}/zwt/occConComView.action?occConCom.id="+row_Id+"&dt="+dt.getTime(),900,400);
            	...
            	<div class="sipac_tableBox">
                	...
                	<table style="width:100%;">
                        ...
                        <tr>
                        	...
                        	<th><span>序号</span></th>
                        	<th><span>职业病危害风险分类 </span></th>
                        	<th><span>所在区域 </span></th>
                        	<th><span>企业名称 </span></th>
                        	<th><span>验收专家 </span></th>
                        	<th><span>行业类别 </span></th>
                        	<th ><span>操作项</span></th>
                        </tr>
  						</thead>
                    	</table>
