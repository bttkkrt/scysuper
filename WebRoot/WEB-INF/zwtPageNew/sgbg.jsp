...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.accidentName)+"</a></td><td>"
		                    		+getSpanByName(item.accidentId)+"</td><td>"
		                    		+getSpanByName(item.accidentTime.substring(0,10))+"</td><td>"+getSpanByName(item.accidentLevel)+"</td><td>"
		                    		+getSpanByName(item.accidentType)+"</td></tr>"
		                    		trs+=tr;
								});
      ...
      function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_opePro","添加事故报告","${ctx}/zwt/opeProInitEdit.action?flag=add&dt="+dt.getTime(),700,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_opePro","修改事故报告","${ctx}/zwt/opeProInitEdit.action?flag=mod&opePro.id="+row_Id+"&dt="+dt.getTime(),700,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_opePro","查看事故报告","${ctx}/zwt/accRepInvHanView.action?accRepInvHan.id="+row_Id+"&dt="+dt.getTime(),700,500);
            	...
            	<div class="sipac_tableBox">
                	...
                	<table style="width:100%;">
                        ...
                        <tr>
                        	...
                        	--><th><span>序号</span></th>
                        	<th><span>事故名称 </span></th>
                        	<th><span>事故编号 </span></th>
                        	<th><span>事故发生时间 </span></th>
                        	<th><span>事故级别 </span></th>
                        	<th><span>事故类别 </span></th>
