    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、备案编号或评价机构名称"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、备案编号或评价机构名称");
		        }
		    });
		});
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		if('${operateRight}'=='add')
		                    		{
		                    			if(item.createUserID == "${sessionScope['LOGIN_USER_ID']}"){
		                    				tr+="<td><input type='checkbox' name='delbox' value='"+item.id+"'></td>";
		                    			}
		                    			else
		                    			{
		                    				tr+="<td>&nbsp;</td>";
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.recordNum)+"</a></td>"
		                    		+ "<td>"+getSpanByName(item.areaId)+"</td>"
		                    		+ "<td>"+getSpanByName(item.companyName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.ratingAgenciesName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.recordDate.substring(0,10))+"</td>";
		                    		if('${operateRight}'=='add')
		                    		{
        ...
        function getButton(rec){
			...
			var button2="";
			if('${operateRight}'=='add' && "${sessionScope['LOGIN_USER_ID']}"==rec.createUserID){
				return  button1;
			}else{
				return button2;
			}
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_assRepFil","添加一般企业安评报告备案","${ctx}/zwt/assRepFilInitEdit.action?flag=add&dt="+dt.getTime(),800,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_assRepFil","修改一般企业安评报告备案","${ctx}/zwt/assRepFilInitEdit.action?flag=mod&assRepFil.id="+row_Id+"&dt="+dt.getTime(),800,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_assRepFil","查看一般企业安评报告备案","${ctx}/zwt/assRepFilView.action?assRepFil.id="+row_Id+"&dt="+dt.getTime(),800,500);
        ...
        var selectState='0';
        function selectAll(){
        	var coll = document.getElementsByName('delbox');
        ...
        <div class="sipac_body" >
            <div class="sipac_container" id="tableDiv" style="margin: 0 0 0 0px;">
            	...
            	<div class="sipac_tableBox">
	                <c:if test="${operateRight=='add'}">
	                    <div class="sipac_tableBar">
	                        <div class="sipac_table_menu"><a href="javascript:addNew();">新建</a><em></em><a href="javascript:del();">批量删除</a><em></em><a href="javascript:refresh();">刷新</a><!--<a href="#">批量删除</a><em></em><a href="#">通过</a><em></em><a href="#">禁用</a><em></em><a href="#">启用</a> --> 
	                       </div>
	                    </div>
