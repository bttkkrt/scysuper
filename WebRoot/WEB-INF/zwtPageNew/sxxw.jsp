...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		if('${roleName}'=='0')
		                    		{
		                    			if(item.createUserID == "${sessionScope['LOGIN_USER_ID']}" && (item.checkStatus=='审核未通过' || item.checkStatus=='审批未通过')){
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.punishName)+"</a></td><td>"
		                    		+getSpanByName(item.areaId)+"</td><td>"+getSpanByName(item.companyName)+"</td><td>"+getSpanByName(item.punishedSpecies)+"</td><td>"+getSpanByName(item.creditRating)+"</td><td>"+getSpanByName(item.checkStatus)+"</td>"
        ...
        function getButton(rec){
         ...
         function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_dishonesty","添加失信行为","${ctx}/zwt/dishonestyInitEdit.action?flag=add&dt="+dt.getTime(),700,550);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_dishonesty","修改失信行为","${ctx}/zwt/dishonestyInitEdit.action?flag=mod&dishonesty.id="+row_Id+"&dt="+dt.getTime(),700,550);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_dishonesty","查看失信行为","${ctx}/zwt/dishonestyView.action?dishonesty.id="+row_Id+"&dt="+dt.getTime(),700,550);
        	
        }
        function check1(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_dishonesty","审核失信行为","${ctx}/zwt/dishonestyCheck1.action?flag=check1&dishonesty.id="+row_Id+"&dt="+dt.getTime(),700,550);
        	
        }
        function check2(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_dishonesty","审批失信行为","${ctx}/zwt/dishonestyCheck2.action?flag=check2&dishonesty.id="+row_Id+"&dt="+dt.getTime(),700,550);
