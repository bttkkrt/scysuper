...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.fileNo)+"</a></td><td>"
		                    		+getSpanByName(item.areaName)+"</td><td>"+getSpanByName(item.companyName)+"</td><td>"+getSpanByName(item.occupationalClassification)+"</td>";
		                    		if('${roleName}'=='1')
		                    		{
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_occConEva","添加职业病危害建设项目预评价审核","${ctx}/zwt/occConEvaInitEdit.action?flag=add&dt="+dt.getTime(),900,400);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_occConEva","修改职业病危害建设项目预评价审核","${ctx}/zwt/occConEvaInitEdit.action?flag=mod&occConEva.id="+row_Id+"&dt="+dt.getTime(),900,400);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_occConEva","查看职业病危害建设项目预评价审核","${ctx}/zwt/occConEvaView.action?occConEva.id="+row_Id+"&dt="+dt.getTime(),900,400);
