...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.fileNo)+"</a></td>"
		                    		+"<td>"+getSpanByName(item.areaId)+"</td>"
		                    		+ "<td>"+getSpanByName(item.companyName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.receptDate.substring(0,10))+"</td>"
		                    		+ "<td>"+getSpanByName(item.receptName)+"</td>";
		                    		if('${operateRight}'=='add')
		                    		{
      ...
      function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_preBusLic","添加非药品类易制毒化学品经营许可证","${ctx}/zwt/preBusLicInitEdit.action?flag=add&dt="+dt.getTime(),900,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_preBusLic","修改非药品类易制毒化学品经营许可证","${ctx}/zwt/preBusLicInitEdit.action?flag=mod&preBusLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_preBusLic","查看非药品类易制毒化学品经营许可证","${ctx}/zwt/preBusLicView.action?preBusLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
            	...
            	<div class="sipac_tableBox">
                	...
                	<table style="width:100%;">
                        ...
                        <tr>
                        	...
                        	<th><span>材料接收日期</span></th>
                        	<th><span>材料接收人员</span></th>
                        	<th ><span>操作项</span></th>
                        </tr>
  						</thead>
                    	</table>
