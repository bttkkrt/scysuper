...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		+getSpanByName(item.planName)+"</a></td><td>"+getSpanByName(item.planType)+"</td><td>"+getSpanByName(item.checkCompanyName)+"</td>"
        ...
        function getButton(rec){
			var button1="<a class='s_t_Btn stb_1' href='javascript:;' onclick=edit('"+rec.id+"')>修改<b></b></a>&nbsp;<a class='s_t_Btn stb_3' href='javascript:;' onclick=link('"+rec.id+"')>关联任务<b></b></a>";
			var button2="<a class='s_t_Btn stb_3' href='javascript:;' onclick=link('"+rec.id+"')>关联任务<b></b></a>";
         ...
         function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_supPla","添加监督检查计划","${ctx}/zwt/supPlaInitEdit.action?flag=add&dt="+dt.getTime(),1000,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supPla","修改监督检查计划","${ctx}/zwt/supPlaInitEdit.action?flag=mod&supPla.id="+row_Id+"&dt="+dt.getTime(),1000,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_supPla","查看监督检查计划","${ctx}/zwt/supPlaView.action?supPla.id="+row_Id+"&dt="+dt.getTime(),1000,500);
        	
        }
        
        function importPlan(){ 
        	createSimpleWindow("importPlan","导入年计划","${ctx}/zwt/initImportPlan.action", 650, 350);
        }
        
        function link(row_Id){
        	var ms=$(".s_sb_nav a");
        	for(var i=0;i<ms.length;i++){
        		if(ms.eq(i).html()=='监督检查计划'){
        			ms.eq(i).removeClass('active');
        		}
        		if(ms.eq(i).html()=='监督检查任务'){
        			ms.eq(i).addClass('active');
        		}
        	}
        	$("#mainContent").load("${ctx}/zwt/jdjcrw.action?supPla.id="+row_Id);
        ...
        <div class="sipac_body" >
            <div class="sipac_container" id="tableDiv" style="margin: 0 0 0 0px;">
            	...
            	<div class="sipac_tableBox">
            		<c:if test="${roleName=='09'}">
	                    <div class="sipac_tableBar">
	                        <div class="sipac_table_menu"><a href="javascript:addNew();">新建</a><em></em><a href="javascript:importPlan();">导入年计划</a><em></em><!--<a href="#">批量删除</a><em></em><a href="#">通过</a><em></em><a href="#">禁用</a><em></em><a href="#">启用</a> --> 
	                       </div>
	                    </div>
	                </c:if>
	               ...
	               <div class="table-head">
                	<table style="width:100%;">
                        ...
                        <tr>
                        	...
                        	<th><span>计划类型</span></th>
                        	<th><span>检查对象名称 </span></th>
                        	<th ><span>操作项</span></th>
                        </tr>
  						</thead>
  					</table>
