...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+agencyName+"</td><td>"+getSpanByName(item.memberNumber)+"</td><td>"+getSpanByName(item.personInCharge)+"</td>"
		                    		+"</tr>";
		                    		trs+=tr;
								});
        ...
        function getName(rec){
			return "<a href='javascript:;' style='color:#27a9e3'  onclick=view('"+rec.id+"')>"+getSpanByName(rec.agencyName)+"<b></b></a>";
        }
        
        
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_emeAge","添加应急机构与队伍","${ctx}/zwt/emeAgeInitEdit.action?flag=add&dt="+dt.getTime(),700,300);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_emeAge","修改应急机构与队伍","${ctx}/zwt/emeAgeInitEdit.action?flag=mod&emeAge.id="+row_Id+"&dt="+dt.getTime(),700,300);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_emeAge","查看应急机构与队伍","${ctx}/zwt/emeAgeView.action?emeAge.id="+row_Id+"&dt="+dt.getTime(),700,300);
