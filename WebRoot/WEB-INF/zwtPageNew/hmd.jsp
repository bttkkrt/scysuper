...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		var tr="";
		                    		if(idx%2==0){
		                    			tr="<tr class='row_1'><td><input type='checkbox' name='delbox' value='"+item.id+"'></td>";
		                    		}else{
		                    			tr="<tr><td><input type='checkbox' name='delbox' value='"+item.id+"'></td>";
		                    		}
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a href='javascript:;' style='color:#27a9e3'  onclick=view('"+item.id+"') >"+getSpanByName(item.illegalActivity)+"</a></td><td>"
		                    		+getSpanByName(item.areaId)+"</td><td>"
		                    		+getSpanByName(item.companyName)+"</td><td>"+getSpanByName(item.startTime.substring(0,11))+"</td></tr>";
		                    		trs+=tr;
								});
        ...
        function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_hmd","添加安全生产黑名单表","${ctx}/zwt/hmdInitEdit.action?flag=add&dt="+dt.getTime(),700,300);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_hmd","修改安全生产黑名单表","${ctx}/zwt/hmdInitEdit.action?flag=mod&hmd.id="+row_Id+"&dt="+dt.getTime(),700,300);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_hmd","查看安全生产黑名单表","${ctx}/zwt/hmdView.action?hmd.id="+row_Id+"&dt="+dt.getTime(),700,300);
            	...
            	<div class="sipac_tableBox">
            		<c:if test="${roleName!='0'}">
	                    <div class="sipac_tableBar">
	                        <div class="sipac_table_menu"><a href="javascript:del();">批量删除</a><!--<em></em><a href="#">修改</a><em></em><a href="#">通过</a><em></em><a href="#">禁用</a><em></em><a href="#">启用</a> --><em></em><a href="javascript:refresh();">刷新</a>
	                       </div>
	                    </div>
	                </c:if>
