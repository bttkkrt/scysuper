    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称或评价机构"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称或评价机构");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.licenseValid.substring(0,10))+"</a></td>"
		                    		+ "<td>"+getSpanByName(item.areaName)+"</td>"
									+ "<td>"+getSpanByName(item.companyName)+"</td>"
									+ "<td>"+getSpanByName(item.ratingAgencies)+"</td>"
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_cheManLic","添加危险化学品经营企业的行政许可","${ctx}/zwt/cheManLicInitEdit.action?flag=add&dt="+dt.getTime(),1100,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheManLic","修改危险化学品经营企业的行政许可","${ctx}/zwt/cheManLicInitEdit.action?flag=mod&cheManLic.id="+row_Id+"&dt="+dt.getTime(),1100,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheManLic","查看危险化学品经营企业的行政许可","${ctx}/zwt/cheManLicView.action?cheManLic.id="+row_Id+"&dt="+dt.getTime(),1100,500);
        	
        }
         function check(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheManLic","审核危险化学品经营企业的行政许可","${ctx}/zwt/cheManLicCheck.action?flag=check&cheManLic.id="+row_Id+"&dt="+dt.getTime(),1100,500);
            	...
            	<div class="sipac_tableBox">
                	...
                	<table style="width:100%;">
                        ...
                        <tr>
                        	...
                        	<th><span>序号</span></th>
                        	<th><span>许可证有效期</span></th>
                        	<th><span>所在区域 </span></th>
                        	<th><span>企业名称 </span></th>
                        	<th><span>评价机构</span></th>
