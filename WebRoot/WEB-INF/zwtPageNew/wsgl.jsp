...
.photos-demo img{width:200px;}
    ...
    var no=1;
    var ps=50;
    var caseId = "${instrumentsInfo.caseId}";
    var ajaxData={ };
    	 $(function(){
    	 	//初始化样式
    	 	if($("#searchType").val()==1){
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		if((item.ifCheck == 3 && item.needCheckUser.indexOf(userid) != -1)
						    		|| (flag.indexOf("A02") != -1 && item.ifCheck == 2)
						    		|| (flag.indexOf("A09") != -1 && item.ifCheck == 1)
						    		|| (flag.indexOf("A30") != -1 && item.ifCheck == 8)
						    		|| (flag.indexOf("A29") != -1))
						    		{
		                    			tr+="<td><input type='checkbox' name='xxx' value='"+item.id+"'></td>";
		                    		}
		                    		else if(flag.indexOf('A02') != -1 || flag.indexOf('A09') != -1 || flag.indexOf('B00') != -1 || flag.indexOf('A30') != -1 || flag.indexOf('A10') != -1 || flag.indexOf('A29') != -1)
		                    		{
		                    			tr+="<td>&nbsp;</td>";
		                    		}
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"
		                    		+getSpanByName(item.instrumentName)+"</a></td><td>"+getSpanByName(item.companyName)+"</td><td>"+getSpanByName(caseStatus)+"</td>"
        ...
        function getButton(rec){
			...
			{
				aa += "<a class='s_t_Btn stb_1' href='javascript:;' onclick=downloadWs('"+rec.id+"')>下载<b></b></a>&nbsp;";
			}
			if((rec.ifCheck == '0' || rec.ifCheck == '5' || rec.ifCheck == '6') && rec.ifPrint == '1')
			{
				aa += "<a class='s_t_Btn stb_4' href='javascript:;' onclick=returnPic('"+rec.id+"')>回执<b></b></a>";
			}
			return aa;
        }
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_instrumentsInfo","添加文书","${ctx}/zwt/instrumentsInfoInitEdit.action?flag=add&dt="+dt.getTime(),900,550);
        	
        }
        function edit(row_Id,type){
        	var dt=new Date();
            createSimpleWindow("win_instrumentsInfo","修改文书","${ctx}/zwt/instrumentsInfoInitEdit.action?flag=mod&instrumentsInfo.id="+row_Id+"&dt="+dt.getTime()+"&type="+type,900,550);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_instrumentsInfo","查看文书","${ctx}/zwt/instrumentsInfoView.action?instrumentsInfo.id="+row_Id+"&dt="+dt.getTime(),900,550);
        	
        }
        function downloadWs(row_Id)
        {
        	document.myform.action = "${ctx}/zwt/instrumentsInfoExport.action?instrumentsInfo.id="+row_Id;
        	document.myform.submit();
        }
        function shenhe(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_instrumentsInfo","审核文书","${ctx}/zwt/instrumentsInfoShenhe.action?instrumentsInfo.id="+row_Id+"&dt="+dt.getTime(),900,550);
        }
        
        function returnPic(row_Id)
        {
        	var dt=new Date();
            createSimpleWindow("win_instrumentsInfo","回执","${ctx}/zwt/instrumentsInfoPic.action?instrumentsInfo.id="+row_Id+"&dt="+dt.getTime(),900,550);
        }
        ...
        function checkPass()
        {
			...
			if(ids == ""){
				...
				var dt=new Date();
            	createSimpleWindow("win_instrumentsInfo","审核文书","${ctx}/zwt/instrumentsInfoShenheAll.action?flag=mod&ids="+ids+"&dt="+dt.getTime()+"&type="+flag,600,300);
			}
        }
        ...
        <div class="sipac_body" >
            <div class="sipac_container" id="tableDiv" style="margin: 0 0 0 0px;">
            	...
            	<div class="sipac_tableBox">
            		 <s:if test="flag.indexOf('A02') != -1 || flag.indexOf('A09') != -1 || flag.indexOf('B00') != -1 || flag.indexOf('A30') != -1 || flag.indexOf('A10') != -1 || flag.indexOf('A29') != -1">
	                    <div class="sipac_tableBar">
	                        <div class="sipac_table_menu">
	                        
	                        <s:if test='%{flag.indexOf("A10") != -1 && caseId == ""}'>
                	...
                	<table style="width:100%;">
                        ...
                        <tr>
                        	...
                        	<th><span>序号</span></th>
                        	<th><span>文书名称 </span></th>
                        	<th><span>企业名称</span></th>
                        	<th><span>文书状态    </span></th>
                        	<th ><span>操作项</span></th>
                        </tr>
  						</thead>
  					</table>
