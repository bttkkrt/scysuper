...
.photos-demo img{width:200px;}
    ...
    var ajaxData={ };
		...
		function getList(num,size){ 
			...
			$("#currPage").val(num);
			$.ajax({
		                	url : "occDisQuery.action?occHazBas.id=${occHazBas.id}",
		                	type: 'post',
		                    dataType: 'json',
		                    async : false,
		                    data:ajaxData,
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+getSpanByName(item.workPlace)+"</td>";
		                    		tr+="<td>"+getSpanByName(item.contactNum)+"</td>";
      ...
      function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_opePro","添加管理制度","${ctx}/zwt/opeProInitEdit.action?flag=add&dt="+dt.getTime(),700,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_opePro","修改管理制度","${ctx}/zwt/opeProInitEdit.action?flag=mod&opePro.id="+row_Id+"&dt="+dt.getTime(),700,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_opePro","查看基本因素","${ctx}/zwt/occDisView.action?occDis.id="+row_Id+"&dt="+dt.getTime(),700,500);
            	...
            	<div class="sipac_tableBox">
                	...
                	<table>
                        ...
                        <tr>
                        	...
                        	--><th><span>序号</span></th>
                        	<th><span>作业场所名称</span></th>
                        	<th><span>接触人数（不可重复）</span></th>
