    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、备案编号或备案机构"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、备案编号或备案机构");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		+ "<td>"+getSpanByName(item.recordAgency)+"</td>"
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_accPlaRec","添加事故应急救援预案备案","${ctx}/zwt/accPlaRecInitEdit.action?flag=add&dt="+dt.getTime(),700,480);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_accPlaRec","修改事故应急救援预案备案","${ctx}/zwt/accPlaRecInitEdit.action?flag=mod&accPlaRec.id="+row_Id+"&dt="+dt.getTime(),700,480);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_accPlaRec","查看事故应急救援预案备案","${ctx}/zwt/accPlaRecView.action?accPlaRec.id="+row_Id+"&dt="+dt.getTime(),700,480);
