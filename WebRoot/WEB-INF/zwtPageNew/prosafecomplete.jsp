    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称、评价单位或审批编号"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称、评价单位或审批编号");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.approvalNum)+"</a></td>"
		                    		+ "<td>"+getSpanByName(item.areaName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.companyName)+"</td>"
		                    		+ "<td>"+getSpanByName(item.evaluationUnit)+"</td>"
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_proSafCom","添加建设项目安全设施施工和竣工验收","${ctx}/zwt/proSafComInitEdit.action?flag=add&dt="+dt.getTime(),900,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proSafCom","修改建设项目安全设施施工和竣工验收","${ctx}/zwt/proSafComInitEdit.action?flag=mod&proSafCom.id="+row_Id+"&dt="+dt.getTime(),900,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_proSafCom","查看建设项目安全设施施工和竣工验收","${ctx}/zwt/proSafComView.action?proSafCom.id="+row_Id+"&dt="+dt.getTime(),900,500);
