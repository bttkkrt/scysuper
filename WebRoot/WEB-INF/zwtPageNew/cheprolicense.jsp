    ...
    var ajaxData={ };
    	 $(function(){
			...
			$('.sipac_searchbar .s_s_text').click(function(){
		        var email = $(this).val();
		        if(email == "搜索企业名称或评价机构名称"){
		            $(this).val("");
		        }
		    }).blur(function(){
		        var email = $(this).val();
		        if(email == ""){
		            $(this).val("搜索企业名称或评价机构名称");
		        }
		    });
		});
			...
			$.ajax({
		                    ...
		                    success: function(data){
		                    	...
		                    	$.each(data.result,function(idx,item){ 
		                    		...
		                    		if('${roleName}'=='1')
		                    		{
		                    			if(item.createUserID == "${sessionScope['LOGIN_USER_ID']}"&& (item.auditState=='审核未通过')){
		                    		...
		                    		tr+="<td>"+index+++"</td>";
		                    		tr+="<td><a onclick=view('"+item.id+"') href='javascript:;' style='color:#27a9e3' >"+getSpanByName(item.ratingAgenciesName)+"</a></td>"
		                    			+"<td>"+getSpanByName(item.areaName)+"</td>"
										+"<td>"+getSpanByName(item.companyName)+"</td>"
										+"<td>"+getSpanByName(item.issuingAuthority)+"</td>"
										+"<td>"+getSpanByName(item.issuingDate.substring(0,10))+"</td>"
										+"<td>"+getSpanByName(item.auditState)+"</td>";
									if('${roleName}'=='1' || "true"=="${canCheck}")
		                    		{
        ...
        function getButton(rec){
        	var button1="<a class='s_t_Btn stb_1' href='javascript:;' onclick=check('"+rec.id+"')>审核<b></b></a>";
			var button2="";
			var button3="<a class='s_t_Btn stb_1' href='javascript:;' onclick=edit('"+rec.id+"')>修改<b></b></a>";
			if(rec.auditState=='待审核'&&"true"=="${canCheck}"){
				return button1;
			}else if(rec.auditState=='审核未通过'&&(rec.createUserID == "${sessionScope['LOGIN_USER_ID']}")&&'${roleName}'=='1'){
				return button3;
			}else{
				return button2;
			}
       ...
       function addNew(){
        	var dt=new Date();
            createSimpleWindow("win_cheProLic","添加危险化学品生产或使用企业安全生产许可证","${ctx}/zwt/cheProLicInitEdit.action?flag=add&dt="+dt.getTime(),900,500);
        	
        }
        function edit(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheProLic","修改危险化学品生产或使用企业安全生产许可证","${ctx}/zwt/cheProLicInitEdit.action?flag=mod&cheProLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
        	
        }
        function view(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheProLic","查看危险化学品生产或使用企业安全生产许可证","${ctx}/zwt/cheProLicView.action?cheProLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
        	
        }
         function check(row_Id){
        	var dt=new Date();
            createSimpleWindow("win_cheProLic","审核危险化学品生产或使用企业安全生产许可证","${ctx}/zwt/cheProLicCheck.action?flag=check&cheProLic.id="+row_Id+"&dt="+dt.getTime(),900,500);
            	...
            	<div class="sipac_tableBox">
	                <c:if test="${roleName=='1'}">
	                    <div class="sipac_tableBar">
	                        <div class="sipac_table_menu"><a href="javascript:addNew();">新建</a><em></em><a href="javascript:del();">批量删除</a><em></em><a href="javascript:refresh();">刷新</a>
	                       </div>
	                    </div>
	                </c:if>
                	...
                	<table style="width:100%;">
                		<colgroup>
                        	...
                        	<col style="width:10%"/>
             				<col style="width:10%"/>
              				...
              				<col style="width:10%"/>
              				<col style="width:10%"/>
              				<c:if test="${roleName=='1' || canCheck == 'true'}">
              				<col style="width:15%"/>
              				</c:if>
          				</colgroup>
                    	<thead>
                        <tr>
                        	...
                        	<th><span>序号</span></th>
                        	<th><span>评价机构名称</span></th>
                        	<th><span>所在区域 </span></th>
                        	<th><span>企业名称 </span></th>
                        	<th><span>发证机关</span></th>
                        	<th><span>发证日期</span></th>
