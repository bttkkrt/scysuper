...
<!--/nav end-->	
<div class="main">
	<div class="location"><p>您现在的位置：<a href="${ctx}/wzxx/index.action">首页</a>&gt;<a href="#">曝光台</a></p></div>
    <div class="news_list">
    <ul>
    	<c:forEach var="bgt" items="${bgtList}">
    	 <c:if test="${fn:length(bgt.title)>40 }">
        	<li><a href="${ctx}/wzxx/bgtContent.action?lighthouse.id=${bgt.id}">${fn:substring(bgt.title, 0, 40)}...</a><fmt:formatDate type="date" value="${bgt.publicDate}" pattern="yyyy-MM-dd"/></li>
        </c:if>
        <c:if test="${fn:length(flfg.infoTitle)<=40 }">
         <li><a href="${ctx}/wzxx/bgtContent.action?lighthouse.id=${bgt.id}">${bgt.title}</a><fmt:formatDate type="date" value="${bgt.publicDate}" pattern="yyyy-MM-dd"/></li>
          </c:if>
        </c:forEach>
    </ul>
    <!--pages-->
