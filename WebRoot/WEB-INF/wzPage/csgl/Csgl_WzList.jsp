...
$(function()
{
        ...
        kkpager.init({
            ...
            //链接前部
            hrefFormer: '${ctx}/wzxx/csglWzList',
            //链接尾部
            hrefLatter: '.action',
            getLink: function (n) {
        ...
        <div class="nav">
             <ul>
               <li><a  href="${ctx}/wzxx/index.action" >首页</a></li>
                ...
                <li><a class="navnow" href="${ctx}/wzxx/csglWzList.action">城市管理</a></li>                 
...
<!--/nav end-->	
<div class="main">
	<div class="location"><p>您现在的位置：<a href="${ctx}/wzxx/index.action">首页</a>&gt;<a href="#">城市管理</a></p></div>
    <div class="news_list">
    <ul>
    	<c:forEach var="csgl" items="${csglList}">
    	  <c:if test="${fn:length(csgl.infoTitle)>40 }">
        	<li><a href="${ctx}/wzxx/csglContent.action?cityMan.id=${csgl.id}">${fn:substring(csgl.infoTitle, 0, 40)}...</a><fmt:formatDate type="date" value="${csgl.publicDate}" pattern="yyyy-MM-dd"/></li>
          </c:if>
           <c:if test="${fn:length(csgl.infoTitle)<=40 }">
           <li><a href="${ctx}/wzxx/csglContent.action?cityMan.id=${csgl.id}">${csgl.infoTitle}</a><fmt:formatDate type="date" value="${csgl.publicDate}" pattern="yyyy-MM-dd"/></li>
            </c:if>
        </c:forEach>
    </ul>
    <!--pages-->
