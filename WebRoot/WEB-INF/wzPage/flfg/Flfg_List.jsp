    ...
    <div id="navBg">
        <div class="nav">
             <ul>
                ...
                <li><a class="navnow" href="${ctx}/wzxx/flfgList.action">法律常规</a></li>             
...
<!--/nav end-->	
<div class="main">
	<div class="location"><p>您现在的位置：<a href="${ctx}/wzxx/index.action">首页</a>&gt;<a href="#">法律常规</a></p></div>
    <div class="news_list">
    
    <div class="title">法律<a href="${ctx}/wzxx/flfgList2.action?law.type=1">更多>></a></div>
    <ul>
    	<c:forEach var="flfg" items="${flfgList1}">
    	  <c:if test="${fn:length(flfg.infoTitle)>40 }">
        	<li><a href="${ctx}/wzxx/flfgContent.action?law.id=${flfg.id}">${fn:substring(flfg.infoTitle, 0, 40)}...</a><fmt:formatDate type="date" value="${flfg.publicTime}" pattern="yyyy-MM-dd"/></li>
          </c:if>
          <c:if test="${fn:length(flfg.infoTitle)<=40 }">
          <li><a href="${ctx}/wzxx/flfgContent.action?law.id=${flfg.id}">${flfg.infoTitle}</a><fmt:formatDate type="date" value="${flfg.publicTime}" pattern="yyyy-MM-dd"/></li>
          </c:if>
        </c:forEach>
    </ul>
     <div class="title">法规<a href="${ctx}/wzxx/flfgList2.action?law.type=2">更多>></a></div>
    <ul>
    	<c:forEach var="flfg" items="${flfgList2}">
    	  <c:if test="${fn:length(flfg.infoTitle)>40 }">
        	<li><a href="${ctx}/wzxx/flfgContent.action?law.id=${flfg.id}">${fn:substring(flfg.infoTitle, 0, 40)}...</a><fmt:formatDate type="date" value="${flfg.publicTime}" pattern="yyyy-MM-dd"/></li>
          </c:if>
          <c:if test="${fn:length(flfg.infoTitle)<=40 }">
      ...
      <div class="title">规章<a href="${ctx}/wzxx/flfgList2.action?law.type=3">更多>></a></div>
      <ul>
    	<c:forEach var="flfg" items="${flfgList3}">
    	  <c:if test="${fn:length(flfg.infoTitle)>40 }">
        	<li><a href="${ctx}/wzxx/flfgContent.action?law.id=${flfg.id}">${fn:substring(flfg.infoTitle, 0, 40)}...</a><fmt:formatDate type="date" value="${flfg.publicTime}" pattern="yyyy-MM-dd"/></li>
          </c:if>
          <c:if test="${fn:length(flfg.infoTitle)<=40 }">
     ...
     <div class="title">其他<a href="${ctx}/wzxx/flfgList2.action?law.type=4">更多>></a></div>
     <ul>
    	<c:forEach var="flfg" items="${flfgList4}">
    	  <c:if test="${fn:length(flfg.infoTitle)>40 }">
        	<li><a href="${ctx}/wzxx/flfgContent.action?law.id=${flfg.id}">${fn:substring(flfg.infoTitle, 0, 40)}...</a><fmt:formatDate type="date" value="${flfg.publicTime}" pattern="yyyy-MM-dd"/></li>
          </c:if>
          <c:if test="${fn:length(flfg.infoTitle)<=40 }">
