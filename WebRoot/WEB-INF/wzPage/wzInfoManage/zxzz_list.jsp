...
$(function()
{
        ...
        kkpager.init({
            ...
            //链接前部
            hrefFormer: '${ctx}/wzxx/awhgzList.action?wzInfoManage.infoType=7',
            //链接尾部
            hrefLatter: '.action',
            getLink: function (n) {
    ...
    <div id="navBg">
        <div class="nav">
             <ul>
                ...
                <li><a class="navnow" href="${ctx}/wzxx/awhgzList.action?wzInfoManage.infoType=7">信息公开</a></li>
...
<!--/nav end-->	
<div class="main">
	<div class="location"><p>您现在的位置：<a href="${ctx}/wzxx/index.action">首页</a>&gt;<a href="#">信息公开</a></p></div>
    <div class="news_list">
    <ul>
    	<c:forEach items="${mapList }" var="map">
			<c:if test="${fn:length(map.infotitle)>40 }">
	    		<li><a href="${ctx}/wzxx/awhgzContent.action?wzInfoManage.id=${map.row_id }" >${fn:substring(map.infotitle, 0, 40)}...</a><fmt:formatDate  pattern="yyyy-MM-dd" type="date" value="${map.createTime}" /></li>
            </c:if>
            <c:if test="${fn:length(map.infotitle)<=40 }">
	    		<li><a href="${ctx}/wzxx/awhgzContent.action?wzInfoManage.id=${map.row_id }" >${map.infotitle }</a><fmt:formatDate  pattern="yyyy-MM-dd" type="date" value="${map.createTime}" /></li>
            </c:if>
      	</c:forEach>
    </ul>
     <!--pages-->
