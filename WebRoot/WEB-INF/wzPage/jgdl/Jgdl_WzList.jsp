...
$(function()
{
        ...
        kkpager.init({
            ...
            //链接前部
            hrefFormer: '${ctx}/wzxx/jgdlWzList',
            //链接尾部
            hrefLatter: '.action',
            getLink: function (n) {
    ...
    <div id="navBg">
        <div class="nav">
             <ul>
                ...
                <li><a  class="navnow" href="${ctx}/wzxx/jgdlWzList.action">机关党建</a></li>
...
<!--/nav end-->	
<div class="main">
	<div class="location"><p>您现在的位置：<a href="${ctx}/wzxx/index.action">首页</a>&gt;<a href="#">机关党建</a></p></div>
    <div class="news_list">
    <ul>
    	<c:forEach var="jgdl" items="${jgdlList}">
    	  <c:if test="${fn:length(jgdl.infoTitle)>40 }">
        	<li><a href="${ctx}/wzxx/jgdlContent.action?jgdl.id=${jgdl.id}">${fn:substring(jgdl.infoTitle, 0, 40)}...</a><fmt:formatDate type="date" value="${jgdl.publicDate}" pattern="yyyy-MM-dd"/></li>
          </c:if>
           <c:if test="${fn:length(aqwh.infoTitle)<=40 }">
           <li><a href="${ctx}/wzxx/jgdlContent.action?jgdl.id=${jgdl.id}">${jgdl.infoTitle}</a><fmt:formatDate type="date" value="${jgdl.publicDate}" pattern="yyyy-MM-dd"/></li>
            </c:if>
        </c:forEach>
    </ul>
    <!--pages-->
