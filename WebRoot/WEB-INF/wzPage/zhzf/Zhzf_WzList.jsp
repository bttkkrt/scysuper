...
$(function()
{
        ...
        kkpager.init({
            ...
            //链接前部
            hrefFormer: '${ctx}/wzxx/zhzfWzList',
            //链接尾部
            hrefLatter: '.action',
            getLink: function (n) {
        ...
        <div class="nav">
             <ul>
               <li><a  href="${ctx}/wzxx/index.action" >首页</a></li>
                ...
                <li><a class="navnow" href="${ctx}/wzxx/zhzfWzList.action">综合执法</a></li>
...
<!--/nav end-->	
<div class="main">
	<div class="location"><p>您现在的位置：<a href="${ctx}/wzxx/index.action">首页</a>&gt;<a href="#">综合执法</a></p></div>
    <div class="news_list">
    <ul>
    	<c:forEach var="zhzf" items="${zhzfList}">
    	  <c:if test="${fn:length(zhzf.infoTitle)>40 }">
        	<li><a href="${ctx}/wzxx/zhzfContent.action?zhzf.id=${zhzf.id}">${fn:substring(zhzf.infoTitle, 0, 40)}...</a><fmt:formatDate type="date" value="${zhzf.publicDate}" pattern="yyyy-MM-dd"/></li>
          </c:if>
           <c:if test="${fn:length(aqwh.infoTitle)<=40 }">
           <li><a href="${ctx}/wzxx/zhzfContent.action?zhzf.id=${zhzf.id}">${zhzf.infoTitle}</a><fmt:formatDate type="date" value="${zhzf.publicDate}" pattern="yyyy-MM-dd"/></li>
            </c:if>
        </c:forEach>
    </ul>
    <!--pages-->
