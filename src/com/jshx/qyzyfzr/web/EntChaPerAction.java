...
public class EntChaPerAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private EntChaPer entChaPer = new EntChaPer();

	/**
	 * 业务类
	 */
	@Autowired
	private EntChaPerService entChaPerService;
	...
	public void list() throws Exception{
		...
		if(null != entChaPer){
			...
			if ((null != entChaPer.getAreaId()) && (0 < entChaPer.getAreaId().trim().length())){
				paraMap.put("areaId", entChaPer.getAreaId().trim());
			}

			if ((null != entChaPer.getCompanyName()) && (0 < entChaPer.getCompanyName().trim().length())){
				paraMap.put("companyName", "%" + entChaPer.getCompanyName().trim() + "%");
			}
			if ((null != entChaPer.getCompanyId()) && (0 < entChaPer.getCompanyId().trim().length())){
				paraMap.put("companyId",   entChaPer.getCompanyId().trim() );
			}
			
			if ((null != entChaPer.getChargeName()) && (0 < entChaPer.getChargeName().trim().length())){
				paraMap.put("chargeName", "%" + entChaPer.getChargeName().trim() + "%");
			}


			if ((null != entChaPer.getChargeSpecializedNum()) && (0 < entChaPer.getChargeSpecializedNum().trim().length())){
				paraMap.put("chargeSpecializedNum", "%" + entChaPer.getChargeSpecializedNum().trim() + "%");
			}
		...
		pagination = entChaPerService.findByPage(pagination, paraMap);
	...
	public String view() throws Exception{
		if((null != entChaPer)&&(null != entChaPer.getId()))
			entChaPer = entChaPerService.getById(entChaPer.getId());
	...
	public String save() throws Exception{
		...
		entChaPer.setAreaId(enBaseInfo.getEnterprisePossession());
		Map m = new HashMap();
		m.put("codeName", "企业属地");
		m.put("itemValue", enBaseInfo.getEnterprisePossession());
		entChaPer.setAreaName(codeService.findCodeValueByMap(m).getItemText());
		entChaPer.setCompanyId(enBaseInfo.getId());
		entChaPer.setCompanyName(enBaseInfo.getEnterpriseName());
		if ("add".equalsIgnoreCase(this.flag)){
			entChaPer.setDeptId(this.getLoginUserDepartmentId());
			entChaPer.setDelFlag(0);
			entChaPerService.save(entChaPer);
		}else{
			entChaPerService.update(entChaPer);
		}
		
		
		
		try {
			String zjhm = NullToString(entChaPer.getChargeSpecializedNum()).replaceAll(" ", "");
			if(zjhm != null && !"".equals(zjhm))
			{
				LkLog log1 = new LkLog();
				LkLog log2 = new LkLog();
				log1.setNrid(entChaPer.getId());
				log1.setLb("entChaPer基本信息");
				log2.setNrid(entChaPer.getId());
				log2.setLb("entChaPer扩展信息");
				...
				NullToString(entChaPer.getChargeName()),//姓名
				NullToString(entChaPer.getChargeSex()),//性别
				gj(entChaPer.getChargeNationnality()),//国籍
				xl(entChaPer.getChargeHighestSchool()),//文化程度
				"企业主要负责人",//职业
				"1");//人员状态
				...
				System.out.println("主要负责人基本信息"+rkhf.getMsg());
				...
				String chargeDutyDate = entChaPer.getChargeDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(entChaPer.getChargeDutyDate()):"";
				String chargePostDate = entChaPer.getChargePostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(entChaPer.getChargePostDate()):"";
				ja.add((new JSONObject()).put("QYMC", NullToString(entChaPer.getCompanyName()) ));//企业名称
				ja.add((new JSONObject()).put("ZGXW", NullToString(entChaPer.getChargeHighestDegree()) ));//最高学位
				ja.add((new JSONObject()).put("BYXY",NullToString(entChaPer.getChargeSchool())));//毕业院校
				ja.add((new JSONObject()).put("ZY" ,NullToString(entChaPer.getChargeSpecialized()) ));//专业
				ja.add((new JSONObject()).put("ZC" ,NullToString(entChaPer.getChargeTitle())));//职称
				ja.add((new JSONObject()).put("AQSCZGZH",NullToString(entChaPer.getChargeSpecializedNum())));//专业资格证号
				ja.add((new JSONObject()).put("LXDH1" , NullToString(entChaPer.getChargePhone()) ));//联系电话
				ja.add((new JSONObject()).put("LXDH2" , NullToString(entChaPer.getChargePhone2()) ));//联系电话2
				ja.add((new JSONObject()).put("EMAIL" ,NullToString(entChaPer.getChargeEmail()) ));//电子邮箱
				ja.add((new JSONObject()).put("ZZ" ,NullToString(entChaPer.getChargeAddress()) ));//住址
				...
				Tkzxxs tkzxxs = ceateTkzxxUtil.ceateTkzxxs("qyzyfzrb",entChaPer.getCreateTime(), kzxx, zjhm, "");
				...
				System.out.println("主要负责人扩展信息"+getTkzxxResponse.getMsg());
	...
	private void setBlobField(FileInputStream in)
	{
		if (null != entChaPer)
		{
			try
			{
				//此处将File对象转换成blob对象，并设置到entChaPer中去
	...
	public String delete() throws Exception{
	    try{
			entChaPerService.deleteWithFlag(ids);
	...
	public EntChaPer getEntChaPer(){
		return this.entChaPer;
	}

	public void setEntChaPer(EntChaPer entChaPer){
		this.entChaPer = entChaPer;
	}
	...
	public void sendInfo()
	{
		...
		try {
			//对接企业主要负责人
			List<EntChaPer> zyfzrlist = entChaPerService.findEntChaPer(null);
			int total2 = zyfzrlist.size();
			int num2 = total2/100;
			int ys2 = total2%100;
			if(ys2 != 0)
			{
			...
			for(int i=0;i<num2;i++)
			{
				...
				{
					EntChaPer zyfzr = zyfzrlist.get(j);
					String zjhm = NullToString(zyfzr.getChargeSpecializedNum()).replaceAll(" ", "").replaceAll(" ", "");
					if(zjhm != null && !"".equals(zjhm))
					{
						Lkrk lkrk = ceateLkRKUtil.ceateLkRK(
						"99",//证件类型
						NullToString(zjhm),//证件号码
						NullToString(zyfzr.getChargeName()),//姓名
						NullToString(zyfzr.getChargeSex()),//性别
						gj(zyfzr.getChargeNationnality()),//国籍
						xl(zyfzr.getChargeHighestSchool()),//文化程度
						"企业主要负责人",//职业
						"1");//人员状态
						rkqq.getLkrk().add(lkrk);
						...
						//人口扩展数据
						JSONArray ja = new JSONArray();
						String chargeDutyDate = zyfzr.getChargeDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(zyfzr.getChargeDutyDate()):"";
						String chargePostDate = zyfzr.getChargePostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(zyfzr.getChargePostDate()):"";
						ja.add((new JSONObject()).put("QYMC", NullToString(zyfzr.getCompanyName()) ));//企业名称
						ja.add((new JSONObject()).put("ZGXW", NullToString(zyfzr.getChargeHighestDegree()) ));//最高学位
						ja.add((new JSONObject()).put("BYXY",NullToString(zyfzr.getChargeSchool())));//毕业院校
						ja.add((new JSONObject()).put("ZY" ,NullToString(zyfzr.getChargeSpecialized()) ));//专业
						ja.add((new JSONObject()).put("ZC" ,NullToString(zyfzr.getChargeTitle())));//职称
						ja.add((new JSONObject()).put("AQSCZGZH",NullToString(zyfzr.getChargeSpecializedNum())));//专业资格证号
						ja.add((new JSONObject()).put("LXDH1" , NullToString(zyfzr.getChargePhone()) ));//联系电话
						ja.add((new JSONObject()).put("LXDH2" , NullToString(zyfzr.getChargePhone2()) ));//联系电话2
						ja.add((new JSONObject()).put("EMAIL" ,NullToString(zyfzr.getChargeEmail()) ));//电子邮箱
						ja.add((new JSONObject()).put("ZZ" ,NullToString(zyfzr.getChargeAddress()) ));//住址
						ja.add((new JSONObject()).put("JRDWSJ" , chargeDutyDate  ));//进入本单位日期
						ja.add((new JSONObject()).put("CSGWSJ" , chargePostDate ));//从事本岗位时间
						JSONObject jo = new JSONObject();
						jo.put("rkkzxx", ja.toString());
