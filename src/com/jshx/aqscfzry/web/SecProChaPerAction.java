...
public class SecProChaPerAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private SecProChaPer secProChaPer = new SecProChaPer();
	...
	private Date queryChargeDutyDateStart;

	private Date queryChargeDutyDateEnd;

	private Date queryChargePostDateStart;

	private Date queryChargePostDateEnd;
	...
	public void list() throws Exception{
		...
		if(null != secProChaPer){
			...
			if ((null != secProChaPer.getAreaId()) && (0 < secProChaPer.getAreaId().trim().length())){
				paraMap.put("areaId", secProChaPer.getAreaId() );
			}

			if ((null != secProChaPer.getChargeSpecializedNum()) && (0 < secProChaPer.getChargeSpecializedNum().trim().length())){
				paraMap.put("chargeSpecializedNum", "%" + secProChaPer.getChargeSpecializedNum().trim() + "%");
			}

			if ((null != secProChaPer.getChargeName()) && (0 < secProChaPer.getChargeName().trim().length())){
				paraMap.put("chargeName", "%" + secProChaPer.getChargeName().trim() + "%");
			}

			if ((null != secProChaPer.getCompanyName()) && (0 < secProChaPer.getCompanyName().trim().length())){
				paraMap.put("companyName", "%" + secProChaPer.getCompanyName().trim() + "%");
			}
			if ((null != secProChaPer.getCompanyId()) && (0 < secProChaPer.getCompanyId().trim().length())){
				paraMap.put("companyId", secProChaPer.getCompanyId().trim());
			}
		...
		final String filter = "id|areaId|companyName|chargeName|chargeSpecializedNum|chargePhone|";
		...
		pagination = secProChaPerService.findByPage(pagination, paraMap);
	...
	public String view() throws Exception{
		if((null != secProChaPer)&&(null != secProChaPer.getId()))
			secProChaPer = secProChaPerService.getById(secProChaPer.getId());
	...
	public String save() throws Exception{
		...
		secProChaPer.setAreaId(enBaseInfo.getEnterprisePossession());
		Map m = new HashMap();
		m.put("codeName", "企业属地");
		m.put("itemValue", enBaseInfo.getEnterprisePossession());
		secProChaPer.setAreaName(codeService.findCodeValueByMap(m).getItemText());
		secProChaPer.setCompanyId(enBaseInfo.getId());
		secProChaPer.setCompanyName(enBaseInfo.getEnterpriseName());
		if ("add".equalsIgnoreCase(this.flag)){
			secProChaPer.setDeptId(this.getLoginUserDepartmentId());
			secProChaPer.setDelFlag(0);
			secProChaPerService.save(secProChaPer);
		}else{
			secProChaPerService.update(secProChaPer);
		}

		try {
			String zjhm = NullToString(secProChaPer.getChargeSpecializedNum()).replaceAll(" ", "");
			if(zjhm != null && !"".equals(zjhm))
			{
				LkLog log1 = new LkLog();
				LkLog log2 = new LkLog();
				log1.setNrid(secProChaPer.getId());
				log1.setLb("secProChaPer基本信息");
				log2.setNrid(secProChaPer.getId());
				log2.setLb("secProChaPer扩展信息");
				...
				NullToString(secProChaPer.getChargeName()),//姓名
				NullToString(secProChaPer.getChargeSex()),//性别
				gj(secProChaPer.getChargeNationnality()),//国籍
				xl(secProChaPer.getChargeHighestSchool()),//文化程度
				"安全生产管理人员",//职业
				"1");//人员状态
				...
				System.out.println("安全生产管理人员基本信息"+rkhf.getMsg());
				...
				String chargeDutyDate = secProChaPer.getChargeDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(secProChaPer.getChargeDutyDate()):"";
				String chargePostDate = secProChaPer.getChargePostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(secProChaPer.getChargePostDate()):"";
				ja.add((new JSONObject()).put("QYMC",NullToString(secProChaPer.getCompanyName())));//企业名称
				ja.add((new JSONObject()).put("ZGXW",NullToString(secProChaPer.getChargeHighestDegree())));//最高学位
				ja.add((new JSONObject()).put("BYXY",NullToString(secProChaPer.getChargeSchool())));//毕业院校
				ja.add((new JSONObject()).put("ZY",NullToString(secProChaPer.getChargeSpecialized())));//专业
				ja.add((new JSONObject()).put("ZC",NullToString(secProChaPer.getChargeTitle())));//职称
				ja.add((new JSONObject()).put("AQSCZGZH",NullToString(secProChaPer.getChargeSpecializedNum())));//专业资格证号
				ja.add((new JSONObject()).put("LXDH1",NullToString(secProChaPer.getChargePhone()) ));//联系电话1
				ja.add((new JSONObject()).put("LXDH2",NullToString(secProChaPer.getChargePhone2()) ));//联系电话2
				ja.add((new JSONObject()).put("EMAIL",NullToString(secProChaPer.getChargeEmail())));//电子邮箱
				ja.add((new JSONObject()).put("ZZ",NullToString(secProChaPer.getChargeAddress())));//住址
				ja.add((new JSONObject()).put("JRDWSJ",chargeDutyDate ));//进入本单位日期
				ja.add((new JSONObject()).put("CSGWSJ",chargePostDate ));//从事本岗位时间
				JSONObject jo = new JSONObject();
				jo.put("rkkzxx", ja.toString());
				String kzxx = jo.toString();
				Tkzxxs tkzxxs = ceateTkzxxUtil.ceateTkzxxs("aqscfzryb",secProChaPer.getCreateTime(), kzxx,zjhm, "");
				...
				System.out.println("安全生产管理人员扩展信息"+getTkzxxResponse.getMsg());
	...
	private void setBlobField(FileInputStream in)
	{
		if (null != secProChaPer)
		{
			try
			{
				//此处将File对象转换成blob对象，并设置到secProChaPer中去
	...
	public String delete() throws Exception{
	    try{
			...
			secProChaPerService.deleteWithFlag(ids);
	 ...
	 * 导入安全生产管理人员
	 */
	public String importSecProChaPer(){
		...
		Sheet sheet = workbook.getSheet(0);
			...
			List<SecProChaPer> secProChaPerList = new ArrayList<SecProChaPer>();
			for (int i = 1; i < row; i++) {
			...
			try {
				...
				if ((cells[0]==null ||cells[0].getContents().equals(""))&&(cells[8]==null ||cells[8].getContents().equals(""))&&(cells[11]==null ||cells[11].getContents().equals("")) ){
					workbook.close();
					break;
				}
				SecProChaPer secProChaPer = new SecProChaPer();
				secProChaPer.setAreaId(enBaseInfo.getEnterprisePossession());
				secProChaPer.setAreaName(codeService.findCodeValueByMap(m).getItemText());
				secProChaPer.setCompanyId(enBaseInfo.getId());
				secProChaPer.setCompanyName(enBaseInfo.getEnterpriseName());
				...
				if (cells[0]==null || cells[0].getContents().equals("")) {
					...
					secProChaPer.setChargeName(cells[0].getContents().trim());
				}
				...
				if (cells[1] != null)
					if(!"".equals(cells[1].getContents().trim())){
						...
						codeMap.put("itemText", cells[1].getContents().trim());
						secProChaPer.setChargeSex(null==codeService.findCodeValueByMap(codeMap).getItemValue()?"":codeService.findCodeValueByMap(codeMap).getItemValue());
					}
				//国籍
				if (cells[2] != null){
					secProChaPer.setChargeNationnality(cells[2].getContents().trim());
				}
				//最高学历
				if (cells[3] != null){
					if(!"".equals(cells[3].getContents().trim())){
						...
						secProChaPer.setChargeHighestSchool(null==codeService.findCodeValueByMap(codeMap).getItemValue()?"":codeService.findCodeValueByMap(codeMap).getItemValue());
					}
				}
				//最高学位
				if (cells[4] != null){
					if(!"".equals(cells[4].getContents().trim())){
						...
						secProChaPer.setChargeHighestDegree(null==codeService.findCodeValueByMap(codeMap).getItemValue()?"":codeService.findCodeValueByMap(codeMap).getItemValue());
				...
				//毕业院校
				if (cells[5] != null){
					secProChaPer.setChargeSchool(cells[5].getContents().trim());
				}
				//专业
				if (cells[6] != null){
					secProChaPer.setChargeSpecialized(cells[6].getContents().trim());
				}
				//职称
				if (cells[7] != null){
					secProChaPer.setChargeTitle(cells[7].getContents().trim());
				}
				//安全生产管理员资格证号
				if (cells[8]==null || cells[8].getContents().equals("")){
					if(colError==null)
						colError = new StringBuffer();
					colError.append("<span style='color:red'>错误：安全生产管理员资格证号输入有误，不能为空，请检查!</span><br>");
				}else{
					secProChaPer.setChargeSpecializedNum(cells[8].getContents().trim());
				}
				
				//进入本单位日期
				if (cells[9] != null){
					if(!"".equals(cells[9].getContents().trim())){
						try {
							if(cells[9].getType() == CellType.DATE)
							{
							     ...
							     sdf.setTimeZone(tz) ; 
							     secProChaPer.setChargeDutyDate(c00.getDate());
							}
				            else
							{
				            	secProChaPer.setChargeDutyDate(sdf.parse(cells[9].getContents().trim()));
							}
				...
				//从事本岗位时间
				if (cells[10] != null){
					if(!"".equals(cells[10].getContents().trim())){
						try {
							if(cells[10].getType() == CellType.DATE)
							{
								 DateCell c00 = (DateCell)cells[10]; 
							     TimeZone tz = TimeZone.getTimeZone("GMT"); 
							     sdf.setTimeZone(tz) ; 
							     secProChaPer.setChargePostDate(c00.getDate());
							}
				            else
							{
				            	secProChaPer.setChargePostDate(sdf.parse(cells[10].getContents().trim()));
							}
						} catch (ParseException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
				...
				//联系电话1
				if (cells[11]==null || cells[11].getContents().equals("")) {
					if(colError==null)
						colError = new StringBuffer();
					colError.append("<span style='color:red'>错误：联系电话1输入有误，不能为空，请检查!</span><br>");
				}else{
					secProChaPer.setChargePhone(cells[11].getContents().trim());
				}
				//联系电话2
				if (cells[12] != null){
					secProChaPer.setChargePhone2(cells[12].getContents().trim());
				}
				//电子邮箱
				if (cells[13] != null){
					secProChaPer.setChargeEmail(cells[13].getContents().trim());
				}
				//住址
				if (cells[14] != null){
					secProChaPer.setChargeAddress(cells[14].getContents().trim());
				}
				//备注
				if (cells[15] != null){
					secProChaPer.setChargeRemark(cells[15].getContents().trim());
				}
				
				if(colError==null){
					secProChaPerList.add(secProChaPer);
		...
		try {
			...
			for(SecProChaPer s:secProChaPerList){
				secProChaPerService.save(s);
				ids.add(s.getId());
			}
			...
			if(ids.size() != 0)
			{
				try {
					...
					Map mm = new HashMap();
					mm.put("ids", ids);
					List<SecProChaPer> fzrlist = secProChaPerService.findSecProChaPer(mm);
					int total6 = fzrlist.size();
					int num6 = total6/100;
					int ys6 = total6%100;
					if(ys6 != 0)
					{
					...
					for(int i=0;i<num6;i++)
					{
						...
						{
							SecProChaPer glry = fzrlist.get(j);
							String zjhm = NullToString(glry.getChargeSpecializedNum()).replaceAll(" ", "");
							if(zjhm != null && !"".equals(zjhm))
							{
								Lkrk lkrk = ceateLkRKUtil.ceateLkRK(
								"99",//证件类型
								NullToString(zjhm),//证件号码
								NullToString(glry.getChargeName()),//姓名
								NullToString(glry.getChargeSex()),//性别
								gj(glry.getChargeNationnality()),//国籍
								xl(glry.getChargeHighestSchool()),//文化程度
								"安全生产管理人员",//职业
								"1");//人员状态
								rkqq.getLkrk().add(lkrk);
								...
								//人口扩展数据
								JSONArray ja = new JSONArray();
								String chargeDutyDate = glry.getChargeDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(glry.getChargeDutyDate()):"";
								String chargePostDate = glry.getChargePostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(glry.getChargePostDate()):"";
								ja.add((new JSONObject()).put("QYMC",NullToString(glry.getCompanyName())));//企业名称
								ja.add((new JSONObject()).put("ZGXW",NullToString(glry.getChargeHighestDegree())));//最高学位
								ja.add((new JSONObject()).put("BYXY",NullToString(glry.getChargeSchool())));//毕业院校
								ja.add((new JSONObject()).put("ZY",NullToString(glry.getChargeSpecialized())));//专业
								ja.add((new JSONObject()).put("ZC",NullToString(glry.getChargeTitle())));//职称
								ja.add((new JSONObject()).put("AQSCZGZH",NullToString(glry.getChargeSpecializedNum())));//专业资格证号
								ja.add((new JSONObject()).put("LXDH1",NullToString(glry.getChargePhone()) ));//联系电话1
								ja.add((new JSONObject()).put("LXDH2",NullToString(glry.getChargePhone2()) ));//联系电话2
								ja.add((new JSONObject()).put("EMAIL",NullToString(glry.getChargeEmail())));//电子邮箱
								ja.add((new JSONObject()).put("ZZ",NullToString(glry.getChargeAddress())));//住址
								ja.add((new JSONObject()).put("JRDWSJ",chargeDutyDate ));//进入本单位日期
								ja.add((new JSONObject()).put("CSGWSJ",chargePostDate ));//从事本岗位时间
								JSONObject jo = new JSONObject();
								jo.put("rkkzxx", ja.toString());
								String kzxx = jo.toString();
								Tkzxxs tkzxxs = ceateTkzxxUtil.ceateTkzxxs("aqscfzryb",glry.getCreateTime(), kzxx, zjhm, "");
								kzqq.getTkzxxs().add(tkzxxs);
							}
						}
		...
		workbook.close();
		if(message.equals("") && secProChaPerList.size() == 0)
		{
	...
	public SecProChaPer getSecProChaPer(){
		return this.secProChaPer;
	}

	public void setSecProChaPer(SecProChaPer secProChaPer){
		this.secProChaPer = secProChaPer;
	}
	...
	public Date getQueryChargeDutyDateStart(){
		return this.queryChargeDutyDateStart;
	}

	public void setQueryChargeDutyDateStart(Date queryChargeDutyDateStart){
		this.queryChargeDutyDateStart = queryChargeDutyDateStart;
	}

	public Date getQueryChargeDutyDateEnd(){
		return this.queryChargeDutyDateEnd;
	}

	public void setQueryChargeDutyDateEnd(Date queryChargeDutyDateEnd){
		this.queryChargeDutyDateEnd = queryChargeDutyDateEnd;
	}

	public Date getQueryChargePostDateStart(){
		return this.queryChargePostDateStart;
	}

	public void setQueryChargePostDateStart(Date queryChargePostDateStart){
		this.queryChargePostDateStart = queryChargePostDateStart;
	}

	public Date getQueryChargePostDateEnd(){
		return this.queryChargePostDateEnd;
	}

	public void setQueryChargePostDateEnd(Date queryChargePostDateEnd){
		this.queryChargePostDateEnd = queryChargePostDateEnd;
	}
	...
	public void sendInfo()
	{
		try {
			//对接安全生产负责人员表
			List<SecProChaPer> fzrlist = secProChaPerService.findSecProChaPer(null);
			int total6 = fzrlist.size();
			int num6 = total6/100;
			int ys6 = total6%100;
			if(ys6 != 0)
			...
			for(int i=0;i<num6;i++)
			{
				...
				LkrekouPort lkrekouPort = lkrekouPortService.getLkrekouPortSoap11();
				GetLkRKResponse rkhf = lkrekouPort.getLkRK(rkqq);
				System.out.println("安全生产管理人员基本信息"+rkhf.getStatus());
				...
				TkzxxPort tkzxxPort = tkzxxPortService.getTkzxxPortSoap11();
				GetTkzxxResponse getTkzxxResponse = tkzxxPort.getTkzxx(kzqq);
				System.out.println("安全生产管理人员扩展信息"+getTkzxxResponse.getStatus());
