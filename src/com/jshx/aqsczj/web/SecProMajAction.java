...
public class SecProMajAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private SecProMaj secProMaj = new SecProMaj();
	...
	private Date queryMajordomoDutyDateStart;

	private Date queryMajordomoDutyDateEnd;

	private Date queryMajordomoPostDateStart;

	private Date queryMajordomoPostDateEnd;
	...
	public void list() throws Exception{
		...
		if(null != secProMaj){
			...
			if ((null != secProMaj.getAreaId()) && (0 < secProMaj.getAreaId().trim().length())){
				paraMap.put("areaId",   secProMaj.getAreaId().trim()  );
			}
			if ((null != secProMaj.getCompanyId()) && (0 < secProMaj.getCompanyId().trim().length())){
				paraMap.put("companyId",   secProMaj.getCompanyId().trim()  );
			}
			if ((null != secProMaj.getCompanyName()) && (0 < secProMaj.getCompanyName().trim().length())){
				paraMap.put("companyName", "%" + secProMaj.getCompanyName().trim() + "%");
			}

			if ((null != secProMaj.getMajordomoSpecializedNum()) && (0 < secProMaj.getMajordomoSpecializedNum().trim().length())){
				paraMap.put("majordomoSpecializedNum", "%" + secProMaj.getMajordomoSpecializedNum().trim() + "%");
			}

			if ((null != secProMaj.getMajordomoCardNum()) && (0 < secProMaj.getMajordomoCardNum().trim().length())){
				paraMap.put("majordomoCardNum", "%" + secProMaj.getMajordomoCardNum().trim() + "%");
			}

			if ((null != secProMaj.getMajordomoName()) && (0 < secProMaj.getMajordomoName().trim().length())){
				paraMap.put("majordomoName", "%" + secProMaj.getMajordomoName().trim() + "%");
			}
		...
		final String filter = "id|areaId|companyName|majordomoName|majordomoPhone|majordomoSpecializedNum|";
		...
		pagination = secProMajService.findByPage(pagination, paraMap);
	...
	public String view() throws Exception{
		if((null != secProMaj)&&(null != secProMaj.getId()))
			secProMaj = secProMajService.getById(secProMaj.getId());
	...
	public String save() throws Exception{
		...
		secProMaj.setAreaId(enBaseInfo.getEnterprisePossession());
		Map m = new HashMap();
		m.put("codeName", "企业属地");
		m.put("itemValue", enBaseInfo.getEnterprisePossession());
		secProMaj.setAreaName(codeService.findCodeValueByMap(m).getItemText());
		secProMaj.setCompanyId(enBaseInfo.getId());
		secProMaj.setCompanyName(enBaseInfo.getEnterpriseName());
		if ("add".equalsIgnoreCase(this.flag)){
			secProMaj.setDeptId(this.getLoginUserDepartmentId());
			secProMaj.setDelFlag(0);
			secProMajService.save(secProMaj);
		}else{
			secProMajService.update(secProMaj);
		}
		
		

		try {
			String zjhm = NullToString(secProMaj.getMajordomoSpecializedNum()).replaceAll(" ", "");
			if(zjhm != null && !"".equals(zjhm))
			{
				LkLog log1 = new LkLog();
				LkLog log2 = new LkLog();
				log1.setNrid(secProMaj.getId());
				log1.setLb("secProMaj基本信息");
				log2.setNrid(secProMaj.getId());
				log2.setLb("secProMaj扩展信息");
				...
				NullToString(secProMaj.getMajordomoName()),//姓名
				NullToString(secProMaj.getMajordomoSex()),//性别
				gj(secProMaj.getMajordomoNationnality()),//国籍
				xl(secProMaj.getMajordomoHighestSchool()),//文化程度
				"安全生产总监",//职业
				"1");//人员状态
				...
				System.out.println("安全生产总监基本信息"+rkhf.getMsg());
				...
				String chargeDutyDate = secProMaj.getMajordomoDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(secProMaj.getMajordomoDutyDate()):"";
				String chargePostDate = secProMaj.getMajordomoPostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(secProMaj.getMajordomoPostDate()):"";
				ja.add((new JSONObject()).put("QYMC",NullToString(secProMaj.getCompanyName())));//企业名称
				ja.add((new JSONObject()).put("ZGXW",NullToString(secProMaj.getMajordomoHighestDegree())));//最高学位
				ja.add((new JSONObject()).put("BYXY",NullToString(secProMaj.getMajordomoSchool())));//毕业院校
				ja.add((new JSONObject()).put("ZY", NullToString(secProMaj.getMajordomoSpecialized())));//专业
				ja.add((new JSONObject()).put("ZC", NullToString(secProMaj.getMajordomoTitle())));//职称
				ja.add((new JSONObject()).put("AQSCZGZH",NullToString(secProMaj.getMajordomoSpecializedNum())));//专业资格证号
				ja.add((new JSONObject()).put("LXDH1",NullToString(secProMaj.getMajordomoPhone())));//联系电话
				ja.add((new JSONObject()).put("LXDH2",NullToString(secProMaj.getMajordomoPhone2())));//联系电话2
				ja.add((new JSONObject()).put("EMAIL",NullToString(secProMaj.getMajordomoEmail())));//电子邮箱
				ja.add((new JSONObject()).put("ZZ",NullToString(secProMaj.getMajordomoAddress())));//住址
				...
				Tkzxxs tkzxxs = ceateTkzxxUtil.ceateTkzxxs("aqsczjb",secProMaj.getCreateTime(), kzxx, zjhm, "");
				...
				System.out.println("安全生产总监扩展信息"+getTkzxxResponse.getMsg());
	...
	private void setBlobField(FileInputStream in)
	{
		if (null != secProMaj)
		{
			try
			{
				//此处将File对象转换成blob对象，并设置到secProMaj中去
	...
	public String delete() throws Exception{
	    try{
			secProMajService.deleteWithFlag(ids);
	...
	public SecProMaj getSecProMaj(){
		return this.secProMaj;
	}

	public void setSecProMaj(SecProMaj secProMaj){
		this.secProMaj = secProMaj;
	}
	...
	public Date getQueryMajordomoDutyDateStart(){
		return this.queryMajordomoDutyDateStart;
	}

	public void setQueryMajordomoDutyDateStart(Date queryMajordomoDutyDateStart){
		this.queryMajordomoDutyDateStart = queryMajordomoDutyDateStart;
	}

	public Date getQueryMajordomoDutyDateEnd(){
		return this.queryMajordomoDutyDateEnd;
	}

	public void setQueryMajordomoDutyDateEnd(Date queryMajordomoDutyDateEnd){
		this.queryMajordomoDutyDateEnd = queryMajordomoDutyDateEnd;
	}

	public Date getQueryMajordomoPostDateStart(){
		return this.queryMajordomoPostDateStart;
	}

	public void setQueryMajordomoPostDateStart(Date queryMajordomoPostDateStart){
		this.queryMajordomoPostDateStart = queryMajordomoPostDateStart;
	}

	public Date getQueryMajordomoPostDateEnd(){
		return this.queryMajordomoPostDateEnd;
	}

	public void setQueryMajordomoPostDateEnd(Date queryMajordomoPostDateEnd){
		this.queryMajordomoPostDateEnd = queryMajordomoPostDateEnd;
	}
	...
	public void sendInfo()
	{
		...
		try {
			//对接安全生产总监表
			List<SecProMaj> zjlist = secProMajService.findSecProMaj(null);
			int total5 = zjlist.size();
			int num5 = total5/100;
			int ys5 = total5%100;
			if(ys5 != 0)
			{
			...
			for(int i=0;i<num5;i++)
			{
				...
				{
					SecProMaj sczj = zjlist.get(j);
					String zjhm = NullToString(sczj.getMajordomoSpecializedNum()).replaceAll(" ", "");
					if(zjhm != null && !"".equals(zjhm))
					{
						Lkrk lkrk = ceateLkRKUtil.ceateLkRK(
						"99",//证件类型
						NullToString(zjhm),//证件号码
						NullToString(sczj.getMajordomoName()),//姓名
						NullToString(sczj.getMajordomoSex()),//性别
						gj(sczj.getMajordomoNationnality()),//国籍
						xl(sczj.getMajordomoHighestSchool()),//文化程度
						"安全生产总监",//职业
						"1");//人员状态
						rkqq.getLkrk().add(lkrk);
						...
						//人口扩展数据
						JSONArray ja = new JSONArray();
						String chargeDutyDate = sczj.getMajordomoDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(sczj.getMajordomoDutyDate()):"";
						String chargePostDate = sczj.getMajordomoPostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(sczj.getMajordomoPostDate()):"";
						ja.add((new JSONObject()).put("QYMC",NullToString(sczj.getCompanyName())));//企业名称
						ja.add((new JSONObject()).put("ZGXW",NullToString(sczj.getMajordomoHighestDegree())));//最高学位
						ja.add((new JSONObject()).put("BYXY",NullToString(sczj.getMajordomoSchool())));//毕业院校
						ja.add((new JSONObject()).put("ZY", NullToString(sczj.getMajordomoSpecialized())));//专业
						ja.add((new JSONObject()).put("ZC", NullToString(sczj.getMajordomoTitle())));//职称
						ja.add((new JSONObject()).put("AQSCZGZH",NullToString(sczj.getMajordomoSpecializedNum())));//专业资格证号
						ja.add((new JSONObject()).put("LXDH1",NullToString(sczj.getMajordomoPhone())));//联系电话
						ja.add((new JSONObject()).put("LXDH2",NullToString(sczj.getMajordomoPhone2())));//联系电话2
						ja.add((new JSONObject()).put("EMAIL",NullToString(sczj.getMajordomoEmail())));//电子邮箱
						ja.add((new JSONObject()).put("ZZ",NullToString(sczj.getMajordomoAddress())));//住址
						ja.add((new JSONObject()).put("JRDWSJ",chargeDutyDate));//进入本单位日期
						ja.add((new JSONObject()).put("CSGWSJ",chargePostDate));//从事本岗位时间
						JSONObject jo = new JSONObject();
						jo.put("rkkzxx", ja.toString());
						String kzxx = jo.toString();
						Tkzxxs tkzxxs = ceateTkzxxUtil.ceateTkzxxs("aqsczjb",sczj.getCreateTime(), kzxx, zjhm, "");
						kzqq.getTkzxxs().add(tkzxxs);
					}
				}
				...
				LkrekouPort lkrekouPort = lkrekouPortService.getLkrekouPortSoap11();
				GetLkRKResponse rkhf = lkrekouPort.getLkRK(rkqq);
				System.out.println("安全生产总监基本信息"+rkhf.getStatus());
				...
				TkzxxPort tkzxxPort = tkzxxPortService.getTkzxxPortSoap11();
				GetTkzxxResponse getTkzxxResponse = tkzxxPort.getTkzxx(kzqq);
				System.out.println("安全生产总监扩展信息"+getTkzxxResponse.getStatus());
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
