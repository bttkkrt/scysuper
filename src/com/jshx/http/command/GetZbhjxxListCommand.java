 ...
 */
public class GetZbhjxxListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					//周边环境类型
					Map m1 = new HashMap();
					m1.put("codeName", "周边环境类型");
					m1.put("itemValue", m.get("SURROUNDING_ENVIRONMENT_TYPE"));
					json.put("surEnvType",null==codeService.findCodeValueByMap(m1).getItemText()?"":codeService.findCodeValueByMap(m1).getItemText() );
					json.put("surEnvName", null==m.get("SURROUNDING_ENVIRONMENT_NAME")?"":m.get("SURROUNDING_ENVIRONMENT_NAME") );
					jArray.add(json);
				}
