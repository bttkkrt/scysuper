 ...
 *
 */
public class GetSgbgListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "ACC_REP_INV_HAN");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> accRepInvHan=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "ACC_REP_INV_HAN");
		...
		try {
				if(accRepInvHan!=null&&!accRepInvHan.isEmpty()){
					for(Map bean:accRepInvHan){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("accidentId", null==bean.get("ACCIDENT_ID")?"":bean.get("ACCIDENT_ID"));//事故编号
						   jo.put("accidentName", null==bean.get("ACCIDENT_NAME")?"":bean.get("ACCIDENT_NAME"));//事故名称
						   jo.put("accidentTime", StringTools.NullToStr(new SimpleDateFormat("yyyy-MM-dd").format(bean.get("ACCIDENT_TIME")),""));
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
