 ...
 */
public class GetXgfListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("workName","");
					json.put("partyName",null==m.get("PARTY_NAME")?"":m.get("PARTY_NAME"));
					jArray.add(json);
				}
