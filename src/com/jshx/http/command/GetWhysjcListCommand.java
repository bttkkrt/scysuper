 ...
 */
public class GetWhysjcListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("deteRiskFactor", null==m.get("DETECTION_RISK_FACTORS")?"":m.get("DETECTION_RISK_FACTORS"));
					json.put("testDate", null==m.get("TEST_DATE")?"":sdf.format(sdf.parse(m.get("TEST_DATE").toString())));
					jArray.add(json);
				}
