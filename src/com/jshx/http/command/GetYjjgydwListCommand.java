 ...
 *
 */
public class GetYjjgydwListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "EME_AGE");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> emeAge=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "EME_AGE");
		...
		try {
				if(emeAge!=null&&!emeAge.isEmpty()){
					for(Map bean:emeAge){
						   JSONObject jo = new JSONObject();
						   jo.put("id", StringTools.NullToStr(bean.get("ROW_ID").toString(),""));//主键
						   jo.put("name", StringTools.NullToStr(bean.get("AGENCY_NAME").toString(),""));//机构名称
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
