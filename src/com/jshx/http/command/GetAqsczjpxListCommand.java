 ...
 * @author 陆妍婷
 */
public class GetAqsczjpxListCommand implements Command
{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		try{
			...
			map.put("table", "MAJORDOMO_TRAINING");
			...
			map1.put("table", "MAJORDOMO_TRAINING");
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("personName",null==m.get("TRAINING_PERSON_NAME")?"":m.get("TRAINING_PERSON_NAME"));
					json.put("cardNum", null==m.get("TRAINING_CARDNUM")?"":m.get("TRAINING_CARDNUM") );
					jArray.add(json);
				}
