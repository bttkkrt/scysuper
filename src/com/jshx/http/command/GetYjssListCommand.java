 ...
 *
 */
public class GetYjssListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "EME_FAC");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> emeFac=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "EME_FAC");
		...
		try {
				if(emeFac!=null&&!emeFac.isEmpty()){
					for(Map bean:emeFac){
						   JSONObject jo = new JSONObject();
						   jo.put("id", StringTools.NullToStr(bean.get("ROW_ID").toString(),""));//主键
						   jo.put("facilityName", StringTools.NullToStr(bean.get("FACILITY_NAME").toString(),""));//物资名称
						   jo.put("number", StringTools.NullToStr(bean.get("FACILITY_NUMBER").toString(),""));//物资数量
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
