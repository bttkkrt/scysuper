 ...
 *
 */
public class GetZyylListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "MAI_MAT");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> maiMat=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "MAI_MAT");
		...
		try {
				if(maiMat!=null&&!maiMat.isEmpty()){
					for(Map bean:maiMat){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("material", null==bean.get("MATERIAL")?"":bean.get("MATERIAL"));//物料
						   jo.put("riskAnalysisi", null==bean.get("RISK_ANALYSIS")?"":bean.get("RISK_ANALYSIS"));//危险性分析
						   jo.put("stoMode", null==bean.get("STORAGE_MODE")?"":bean.get("STORAGE_MODE"));//存放方式
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
