...
public class GetAllMajorTroubleListCommand implements Command
{
	...
	public BaseResponse execute(JSONObject json) {
		...
		List<CommonTroubleBean> commons = httpService.getAllMajorTrouble(paraMap);//获取重大安全隐患列表信息

		int total = httpService.getAllMajorTroubleCount(paraMap);//获取重大安全隐患总条数
		int page=total/(l);
		...
		try {
				if(commons!=null&&!commons.isEmpty()){
					for(CommonTroubleBean bean:commons){
						   ...
						   jo.put("gpsj", StringTools.NullToStr(bean.getGpsj(),""));//挂牌时间
