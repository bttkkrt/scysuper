...
public class GetJccbListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "DET_OVER_STA_MAN");
		...
		paraMap2.put("table", "DET_OVER_STA_MAN");
		...
		try {
				if(secProIns!=null&&!secProIns.isEmpty()){
					for(Map bean:secProIns){
						   ...
						   jo.put("samEncode", null==bean.get("SAMPLE_ENCODING")?"":bean.get("SAMPLE_ENCODING"));//样品编码
						   jo.put("testItems", null==bean.get("TEST_ITEMS")?"":bean.get("TEST_ITEMS"));//检测项目
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
