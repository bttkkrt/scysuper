 ...
 *
 */
public class GetYjyaListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "EME_PLA");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> emePla=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "EME_PLA");
		...
		try {
				if(emePla!=null&&!emePla.isEmpty()){
					for(Map bean:emePla){
						   JSONObject jo = new JSONObject();
						   jo.put("id",null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("planName", null==bean.get("PLAN_NAME")?"":bean.get("PLAN_NAME"));//预案名称
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
