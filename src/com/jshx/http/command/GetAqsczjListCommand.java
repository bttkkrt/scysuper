 ...
 */
public class GetAqsczjListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("name",null==m.get("MAJORDOMO_NAME")?"":m.get("MAJORDOMO_NAME"));
					json.put("specializeNum", null==m.get("MAJORDOMO_SPECIALIZED_NUM")?"":m.get("MAJORDOMO_SPECIALIZED_NUM") );
					jArray.add(json);
				}
