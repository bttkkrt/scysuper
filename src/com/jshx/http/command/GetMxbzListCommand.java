 ...
 *
 */
public class GetMxbzListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "SIGNS");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> signs=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "SIGNS");
		...
		try {
				if(signs!=null&&!signs.isEmpty()){
					for(Map bean:signs){
						   ...
						   jo.put("name", null==bean.get("SIGNS_NAME")?"":bean.get("SIGNS_NAME"));//清单名称
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
