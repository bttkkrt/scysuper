 ...
 */
public class GetLbyplyListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("time",null==m.get("PPE_USE_TIME")?"":sdf.format(sdf.parse(m.get("PPE_USE_TIME").toString())));
