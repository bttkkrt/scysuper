 ...
 */
public class GetSccjListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("workCode",null==m.get("WORKSHOP_CODE")?"":m.get("WORKSHOP_CODE"));
					json.put("workName", null==m.get("WORKSHOP_NAME")?"":m.get("WORKSHOP_NAME") );
					jArray.add(json);
				}
