 ...
 * @author 陆妍婷
 */
public class GetNdzybfzjfListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("year",null==m.get("JSHX_YEAR")?"":m.get("JSHX_YEAR"));
					json.put("attachment",null==m.get("ATTACHMENT")?"":m.get("ATTACHMENT"));
					jArray.add(json);
				}
