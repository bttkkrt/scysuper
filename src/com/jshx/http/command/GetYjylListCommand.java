 ...
 *
 */
public class GetYjylListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "EME_DRI");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> emeDri=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "EME_DRI");
		...
		try {
				if(emeDri!=null&&!emeDri.isEmpty()){
					for(Map bean:emeDri){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("drillName", null==bean.get("DRILL_NAME")?"":bean.get("DRILL_NAME"));//演练名称
						   Map m = new HashMap();
						   m.put("codeName", "应急演练类型");
							m.put("itemValue", bean.get("DRILL_TYPE").toString());
							String type=codeService.findCodeValueByMap(m).getItemText();
						   jo.put("type", type);//演练类型
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
