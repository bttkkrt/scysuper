 ...
 *
 */
public class GetQyzyfzrlzqkbgListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "MAI_PER_REP");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> maiPerRep=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "MAI_PER_REP");
		...
		try {
				if(maiPerRep!=null&&!maiPerRep.isEmpty()){
					for(Map bean:maiPerRep){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("priResPerson",null==bean.get("PRINCIPAL_RESPONSIBLE_PERSON")?"":bean.get("PRINCIPAL_RESPONSIBLE_PERSON"));//主要负责人
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
