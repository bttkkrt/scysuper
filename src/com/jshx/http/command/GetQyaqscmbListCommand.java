 ...
 * @author 陆妍婷
 */
public class GetQyaqscmbListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("proYear",null==m.get("PRODUCT_GOAL_YEAR")?"":new SimpleDateFormat("yyyy").format(m.get("PRODUCT_GOAL_YEAR")));
					jArray.add(json);
				}
