 ...
 */
public class GetWhaqglzdListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("name",null==m.get("SYSTEM_NAME")?"":m.get("SYSTEM_NAME"));
					json.put("time",null==m.get("UPLOAD_TIME")?"":sdf.format(sdf.parse(m.get("UPLOAD_TIME").toString())));
					
					//危化安全管理制度类型
					Map m0 = new HashMap();
					m0.put("codeName", "危化安全管理制度类型");
					m0.put("itemValue",m.get("SYSTEM_TYPE").toString());
					json.put("type",null==codeService.findCodeValueByMap(m0).getItemText()?"":codeService.findCodeValueByMap(m0).getItemText() );
					
					//附件
					Map<String,Object> mapFJ=new HashMap<String, Object>();
					mapFJ.put("picType", "whaqglzdfj");
					mapFJ.put("linkId", m.get("LINK_ID").toString());
					mapFJ.put("mkType", "whaqglzd");
