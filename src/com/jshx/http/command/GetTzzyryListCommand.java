 ...
 */
public class GetTzzyryListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("name",null==m.get("SPECIAL_NAME")?"":m.get("SPECIAL_NAME"));
					json.put("jobCardNum", null==m.get("SPECIAL_JOB_CRADNUM")?"":m.get("SPECIAL_JOB_CRADNUM") );
					jArray.add(json);
				}
