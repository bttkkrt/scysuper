 ...
 */
public class GetZdwhpclqkListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("dangerCheName",null==m.get("DANGEROUS_CHEMICAL_NAME")?"":m.get("DANGEROUS_CHEMICAL_NAME"));
					json.put("storage",null==m.get("STORAGE")?"":m.get("STORAGE"));
					jArray.add(json);
				}
