 ...
 */
public class GetScsbssListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("code",null==m.get("DEVICE_CODE")?"":m.get("DEVICE_CODE"));
					json.put("name", null==m.get("DEVICE_NAME")?"":m.get("DEVICE_NAME") );
					jArray.add(json);
				}
