 ...
 */
public class GetAqscldjgListCommand implements Command
{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		try{
			...
			map.put("table", "PRO_LEG_ORG");
			map.put("sqlId", "getListByTableAndCompanyId");
			map.put("start", pageNum);
			map.put("limit", pageSize);
			List<Map<String,Object>> aqscldjgList=httpService.findListDataByMap(map);
			//查询总数
			Map<String,Object>  map1 = new HashMap<String,Object> ();
			map1.put("companyId", companyId);
			map1.put("table", "PRO_LEG_ORG");
			...
			//查询成功
			if(aqscldjgList.size()>0){
				bd.setCode("0");
				bd.setMessage("查询成功");
				JSONArray jArray=new JSONArray();
				for(Map<String,Object> m:aqscldjgList){
					JSONObject json = new JSONObject();
					json.put("id",null==m.get("ROW_ID")?"":m.get("ROW_ID"));
					json.put("orgenName", null==m.get("ORGENIZATION_NAME")?"":m.get("ORGENIZATION_NAME"));
					jArray.add(json);
				}
