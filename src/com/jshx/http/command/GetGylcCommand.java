 ...
 *
 */
public class GetGylcCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "TEC_PRO");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> tecPro=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "TEC_PRO");
		...
		try {
				if(tecPro!=null&&!tecPro.isEmpty()){
					for(Map bean:tecPro){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("proProcess",null==bean.get("PRODUCTION_PROCESS_NAME")?"":bean.get("PRODUCTION_PROCESS_NAME"));//生产工艺
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
