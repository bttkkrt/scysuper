 ...
 */
public class GetLbypkfListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("name",null==m.get("PPE_NAME")?"":m.get("PPE_NAME"));
					Map m1 = new HashMap();
					m1.put("codeName", "进库出库");
					m1.put("itemValue", m.get("PPE_WARE_TYPE"));
					json.put("type",null==codeService.findCodeValueByMap(m1).getItemText()?"":codeService.findCodeValueByMap(m1).getItemText()  );
					jArray.add(json);
				}
