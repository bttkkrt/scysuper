 ...
 *
 */
public class GetQyzdwxyglzdListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "COM_MAN_SYS");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> comManSys=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "COM_MAN_SYS");
		...
		try {
				if(comManSys!=null&&!comManSys.isEmpty()){
					for(Map bean:comManSys){
						   JSONObject jo = new JSONObject();
						   jo.put("id", StringTools.NullToStr(bean.get("ROW_ID").toString(),""));//主键
						   jo.put("systemName", StringTools.NullToStr(bean.get("SYSTEM_NAME").toString(),""));//制度名称
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
