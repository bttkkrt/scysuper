 ...
 */
public class GetSbsskjListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("number",null==m.get("EQUIPMENT_NUMBER")?"":m.get("EQUIPMENT_NUMBER"));
					json.put("name", null==m.get("EQUIPMENT_NAME")?"":m.get("EQUIPMENT_NAME") );
					json.put("place", null==m.get("EQUIPMENT_PLACE")?"":m.get("EQUIPMENT_PLACE") );
					jArray.add(json);
				}
