 ...
 */
public class GetZyjktjListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("createTime",null==m.get("CREATETIME")?"":sdf.format(sdf.parse(m.get("CREATETIME").toString())));
					json.put("medInstitute",null==m.get("MEDICAL_INSTITUTION_NAME")?"":m.get("MEDICAL_INSTITUTION_NAME"));
					jArray.add(json);
				}
