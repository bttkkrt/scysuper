 ...
 *
 */
public class GetGglListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "PUBLIC_BOARD");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> publicBoard=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "PUBLIC_BOARD");
		...
		try {
				if(publicBoard!=null&&!publicBoard.isEmpty()){
					for(Map bean:publicBoard){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("type", null==bean.get("PUBLIC_TYPE")?"":bean.get("PUBLIC_TYPE"));//公告栏编号
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
