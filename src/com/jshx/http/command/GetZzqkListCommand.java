 ...
 * @author 陆妍婷
 */
public class GetZzqkListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("intelCardNum",null==m.get("INTELLIGENCE_CARDNUM")?"":m.get("INTELLIGENCE_CARDNUM"));
					json.put("intelCardName", null==m.get("INTELLIGENCE_CARDNAME")?"":m.get("INTELLIGENCE_CARDNAME"));
					jArray.add(json);
				}
