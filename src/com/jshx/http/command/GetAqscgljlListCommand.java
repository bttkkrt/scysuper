 ...
 */
public class GetAqscgljlListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("name",null==m.get("MANAGER_NAME")?"":m.get("MANAGER_NAME"));
					json.put("specializeNum", null==m.get("MANAGER_SPECIALIZED_NUM")?"":m.get("MANAGER_SPECIALIZED_NUM") );
					jArray.add(json);
				}
