 ...
 *
 */
public class GetQyzdwxyyjczListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "COM_DAN_EME");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> comDanEme=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "COM_DAN_EME");
		...
		try {
				if(comDanEme!=null&&!comDanEme.isEmpty()){
					for(Map bean:comDanEme){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("emerName", null==bean.get("EMERGENCY_NAME")?"":bean.get("EMERGENCY_NAME"));//应急处置名称
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
