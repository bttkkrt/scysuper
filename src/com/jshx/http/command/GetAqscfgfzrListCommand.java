 ...
 */
public class GetAqscfgfzrListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("name",null==m.get("CHARGER_NAME")?"":m.get("CHARGER_NAME"));
					json.put("specializeNum", null==m.get("CHARGER_SPECIALIZED_NUM")?"":m.get("CHARGER_SPECIALIZED_NUM") );
					jArray.add(json);
				}
