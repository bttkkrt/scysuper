 ...
 */
public class GetSwwzxxListCommand implements Command
{
		...
		try{
			...
			if(aqscgljgList.size()>0){
				...
				for(Map<String,Object> m:aqscgljgList){
					...
					json.put("danCheName",null==m.get("DANGEROUS_CHEMICAL_NAME")?"":m.get("DANGEROUS_CHEMICAL_NAME"));
					json.put("riskGauge", null==m.get("RISK_GAUGE")?"":m.get("RISK_GAUGE") );
					jArray.add(json);
				}
