 ...
 *
 */
public class GetAqscgzzdListCommand implements Command{
	...
	public BaseResponse execute(JSONObject obj) {
		...
		paraMap.put("table", "SEC_PRO_SYS");
		paraMap.put("sqlId", "getListByTableAndCompanyId");
		paraMap.put("start", pageNum);
		paraMap.put("limit", pageSize);
		paraMap.put("companyId", companyId);

		List<Map<String, Object>> secProSys=httpService.findListDataByMap(paraMap);
		Map<String, Object> paraMap2 = new HashMap<String, Object>();
		paraMap2.put("table", "SEC_PRO_SYS");
		...
		try {
				if(secProSys!=null&&!secProSys.isEmpty()){
					for(Map bean:secProSys){
						   JSONObject jo = new JSONObject();
						   jo.put("id", null==bean.get("ROW_ID")?"":bean.get("ROW_ID"));//主键
						   jo.put("systemName", null==bean.get("SYSTEM_NAME")?"":bean.get("SYSTEM_NAME"));//制度名称
						   jo.put("systemTime", null==bean.get("SYSTEM_TIME")?"":new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(bean.get("SYSTEM_TIME")));//上传时间
						   ja.add(jo);
					}
					br.setCode("0");
					br.setMessage("成功");
