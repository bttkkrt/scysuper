...
public class SecProManAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private SecProMan secProMan = new SecProMan();
	...
	private Date queryManagerDutyDateStart;

	private Date queryManagerDutyDateEnd;

	private Date queryManagerPostDateStart;

	private Date queryManagerPostDateEnd;
	...
	public void list() throws Exception{
		...
		if(null != secProMan){
			...
			if ((null != secProMan.getAreaId()) && (0 < secProMan.getAreaId().trim().length())){
				paraMap.put("areaId", secProMan.getAreaId().trim());
			}
			if ((null != secProMan.getCompanyId()) && (0 < secProMan.getCompanyId().trim().length())){
				paraMap.put("companyId", secProMan.getCompanyId().trim());
			}

			if ((null != secProMan.getManagerSpecializedNum()) && (0 < secProMan.getManagerSpecializedNum().trim().length())){
				paraMap.put("managerSpecializedNum", "%" + secProMan.getManagerSpecializedNum().trim() + "%");
			}

			if ((null != secProMan.getManagerName()) && (0 < secProMan.getManagerName().trim().length())){
				paraMap.put("managerName", "%" + secProMan.getManagerName().trim() + "%");
			}

			if ((null != secProMan.getCompanyName()) && (0 < secProMan.getCompanyName().trim().length())){
				paraMap.put("companyName", "%" + secProMan.getCompanyName().trim() + "%");
			}
		...
		final String filter = "id|areaId|companyName|managerName|managerSpecializedNum|managerPhone|";
		...
		pagination = secProManService.findByPage(pagination, paraMap);
	...
	public String view() throws Exception{
		if((null != secProMan)&&(null != secProMan.getId()))
			secProMan = secProManService.getById(secProMan.getId());
	...
	public String save() throws Exception{
		...
		secProMan.setAreaId(enBaseInfo.getEnterprisePossession());
		Map m = new HashMap();
		m.put("codeName", "企业属地");
		m.put("itemValue", enBaseInfo.getEnterprisePossession());
		secProMan.setAreaName(codeService.findCodeValueByMap(m).getItemText());
		secProMan.setCompanyId(enBaseInfo.getId());
		secProMan.setCompanyName(enBaseInfo.getEnterpriseName());
		if ("add".equalsIgnoreCase(this.flag)){
			secProMan.setDeptId(this.getLoginUserDepartmentId());
			secProMan.setDelFlag(0);
			secProManService.save(secProMan);
		}else{
			secProManService.update(secProMan);
		}
		
		try {
			String zjhm = NullToString(secProMan.getManagerSpecializedNum()).replaceAll(" ", "");
			if(zjhm != null && !"".equals(zjhm))
			{
				LkLog log1 = new LkLog();
				LkLog log2 = new LkLog();
				log1.setNrid(secProMan.getId());
				log1.setLb("secProMan基本信息");
				log2.setNrid(secProMan.getId());
				log2.setLb("secProMan扩展信息");
				...
				Lkrk lkrk = ceateLkRKUtil.ceateLkRK(
					...
					NullToString(secProMan.getManagerName()),//姓名
					NullToString(secProMan.getManagerSex()),//性别
					gj(secProMan.getManagerNationnality()),//国籍
					xl(secProMan.getManagerHighestSchool()),//文化程度
					"安全生产管理经理",//职业
					"1");//人员状态
				...
				System.out.println("安全生产管理经理基本信息"+rkhf.getMsg());
				...
				String chargeDutyDate = secProMan.getManagerDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(secProMan.getManagerDutyDate()):"";
				String chargePostDate = secProMan.getManagerPostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(secProMan.getManagerPostDate()):"";
				ja.add((new JSONObject()).put("QYMC",NullToString(secProMan.getCompanyName())));//企业名称
				ja.add((new JSONObject()).put("ZGXW",NullToString(secProMan.getManagerHighestDegree())));//最高学位
				ja.add((new JSONObject()).put("BYXY",NullToString(secProMan.getManagerSchool())));//毕业院校
				ja.add((new JSONObject()).put("ZY",NullToString(secProMan.getManagerSpecialized()) ));//专业
				ja.add((new JSONObject()).put("ZC",NullToString(secProMan.getManagerTitle())));//职称
				ja.add((new JSONObject()).put("AQSCZGZH",NullToString(secProMan.getManagerSpecializedNum())));//专业资格证号
				ja.add((new JSONObject()).put("LXDH1", NullToString(secProMan.getManagerPhone())));//联系电话1
				ja.add((new JSONObject()).put("LXDH2", NullToString(secProMan.getManagerPhone2())));//联系电话2
				ja.add((new JSONObject()).put("EMAIL", NullToString(secProMan.getManagerEmail())));//电子邮箱
				ja.add((new JSONObject()).put("ZZ", NullToString(secProMan.getManagerAddress()) ));//住址
				...
				Tkzxxs tkzxxs = ceateTkzxxUtil.ceateTkzxxs("aqscgljlb",secProMan.getCreateTime(), kzxx,zjhm, "");
				...
				System.out.println("安全生产管理经理扩展信息"+getTkzxxResponse.getMsg());
	...
	private void setBlobField(FileInputStream in)
	{
		if (null != secProMan)
		{
			try
			{
				//此处将File对象转换成blob对象，并设置到secProMan中去
	...
	public String delete() throws Exception{
	    try{
			secProManService.deleteWithFlag(ids);
	...
	public SecProMan getSecProMan(){
		return this.secProMan;
	}

	public void setSecProMan(SecProMan secProMan){
		this.secProMan = secProMan;
	}
	...
	public Date getQueryManagerDutyDateStart(){
		return this.queryManagerDutyDateStart;
	}

	public void setQueryManagerDutyDateStart(Date queryManagerDutyDateStart){
		this.queryManagerDutyDateStart = queryManagerDutyDateStart;
	}

	public Date getQueryManagerDutyDateEnd(){
		return this.queryManagerDutyDateEnd;
	}

	public void setQueryManagerDutyDateEnd(Date queryManagerDutyDateEnd){
		this.queryManagerDutyDateEnd = queryManagerDutyDateEnd;
	}

	public Date getQueryManagerPostDateStart(){
		return this.queryManagerPostDateStart;
	}

	public void setQueryManagerPostDateStart(Date queryManagerPostDateStart){
		this.queryManagerPostDateStart = queryManagerPostDateStart;
	}

	public Date getQueryManagerPostDateEnd(){
		return this.queryManagerPostDateEnd;
	}

	public void setQueryManagerPostDateEnd(Date queryManagerPostDateEnd){
		this.queryManagerPostDateEnd = queryManagerPostDateEnd;
	}
	...
	public void sendInfo()
	{
		try {
			//对接安全生产管理经理表
			List<SecProMan> gljllist = secProManService.findSecProMan(null);
			int total3 = gljllist.size();
			int num3 = total3/100;
			int ys3 = total3%100;
			if(ys3 != 0)
			{
			...
			for(int i=0;i<num3;i++)
			{
				...
				{
					SecProMan gljl = gljllist.get(j);
					String zjhm = NullToString(gljl.getManagerSpecializedNum()).replaceAll(" ", "");
					if(zjhm != null && !"".equals(zjhm))
					{
						Lkrk lkrk = ceateLkRKUtil.ceateLkRK(
							"99",//证件类型
							NullToString(zjhm),//证件号码
							NullToString(gljl.getManagerName()),//姓名
							NullToString(gljl.getManagerSex()),//性别
							gj(gljl.getManagerNationnality()),//国籍
							xl(gljl.getManagerHighestSchool()),//文化程度
							"安全生产管理经理",//职业
							"1");//人员状态
						rkqq.getLkrk().add(lkrk);
						...
						//人口扩展数据
						JSONArray ja = new JSONArray();
						String chargeDutyDate = gljl.getManagerDutyDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(gljl.getManagerDutyDate()):"";
						String chargePostDate = gljl.getManagerPostDate() != null?new SimpleDateFormat("yyyy-MM-dd").format(gljl.getManagerPostDate()):"";
						ja.add((new JSONObject()).put("QYMC",NullToString(gljl.getCompanyName())));//企业名称
						ja.add((new JSONObject()).put("ZGXW",NullToString(gljl.getManagerHighestDegree())));//最高学位
						ja.add((new JSONObject()).put("BYXY",NullToString(gljl.getManagerSchool())));//毕业院校
						ja.add((new JSONObject()).put("ZY",NullToString(gljl.getManagerSpecialized()) ));//专业
						ja.add((new JSONObject()).put("ZC",NullToString(gljl.getManagerTitle())));//职称
						ja.add((new JSONObject()).put("AQSCZGZH",NullToString(gljl.getManagerSpecializedNum())));//专业资格证号
						ja.add((new JSONObject()).put("LXDH1", NullToString(gljl.getManagerPhone())));//联系电话1
						ja.add((new JSONObject()).put("LXDH2", NullToString(gljl.getManagerPhone2())));//联系电话2
						ja.add((new JSONObject()).put("EMAIL", NullToString(gljl.getManagerEmail())));//电子邮箱
						ja.add((new JSONObject()).put("ZZ", NullToString(gljl.getManagerAddress()) ));//住址
						ja.add((new JSONObject()).put("JRDWSJ",chargeDutyDate ));//进入本单位日期
						ja.add((new JSONObject()).put("CSGWSJ", chargePostDate ));//从事本岗位时间
						JSONObject jo = new JSONObject();
						jo.put("rkkzxx", ja.toString());
						String kzxx = jo.toString();
						Tkzxxs tkzxxs = ceateTkzxxUtil.ceateTkzxxs("aqscgljlb",gljl.getCreateTime(), kzxx, zjhm, "");
						kzqq.getTkzxxs().add(tkzxxs);
				...
				LkrekouPort lkrekouPort = lkrekouPortService.getLkrekouPortSoap11();
				GetLkRKResponse rkhf = lkrekouPort.getLkRK(rkqq);
				System.out.println("安全生产管理经理基本信息"+rkhf.getStatus()+rkhf.getMsg());
				...
				TkzxxPort tkzxxPort = tkzxxPortService.getTkzxxPortSoap11();
				GetTkzxxResponse getTkzxxResponse = tkzxxPort.getTkzxx(kzqq);
				System.out.println("安全生产管理经理扩展信息"+getTkzxxResponse.getStatus()+getTkzxxResponse.getMsg());
