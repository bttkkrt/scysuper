/**
 * Class Name: OccupationalControlPlanAction
 * Class Description：职业病危害控制效果评价报告表备案通知书
 * Writer：陆婷
 * CreateTime：2013-11-18
 */
package com.jshx.occupationalControlPlan.web;
...
public class OccupationalControlPlanAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private OccupationalControlPlan occupationalControlPlan = new OccupationalControlPlan();
	...
	/**
	 * 新增查询字段  lj 2014-06-26
	 * start
	 */
	private String wcrqStart; //报告完成日期开始
	private String wcrqEnd;//报告完成日期结束
	private String sbrqStart;//上报日期开始
	private String sbrqEnd;//上报日期结束
	/**
	 * end
	 */
	...
	/**
	 * Function Name: init
	 * Function Description：初始化职业病危害控制效果评价报告表备案通知书列表
	 * Writer：陆婷
	 * CreateTime：2013-11-18
	 */
	public String init(){
	...
	/**
	 * Function Name: list
	 * Function Description：查询职业病危害控制效果评价报告表备案通知书列表
	 * Writer：陆婷
	 * CreateTime：2013-11-18
	 */
	public void list() throws Exception{
		...
		if(null != occupationalControlPlan){
		    //设置查询条件，开发人员可以在此增加过滤条件
			if ((null != occupationalControlPlan.getCompanyName()) && (0 < occupationalControlPlan.getCompanyName().trim().length())){
				paraMap.put("companyName", "%" + occupationalControlPlan.getCompanyName().trim() + "%");
			}
			if ((null != occupationalControlPlan.getSzzid()) && (0 < occupationalControlPlan.getSzzid().trim().length())){
				paraMap.put("szzid", occupationalControlPlan.getSzzid().trim());
			}
			if ((null != occupationalControlPlan.getRequiredThree()) && (0 < occupationalControlPlan.getRequiredThree().trim().length())){
				paraMap.put("requiredThree", occupationalControlPlan.getRequiredThree().trim());
			}
		}

		if (null != wcrqStart){
			paraMap.put("wcrqStart", wcrqStart);
		}
		if (null != wcrqEnd){
			paraMap.put("wcrqEnd", wcrqEnd);
		}
		if (null != sbrqStart){
			paraMap.put("sbrqStart", sbrqStart);
		}
		if (null != sbrqEnd){
			paraMap.put("sbrqEnd", sbrqEnd);
		}
	...
	/**
	 * Function Name: view
	 * Function Description：查看职业病危害控制效果评价报告表备案通知书信息
	 * Writer：陆婷
	 * CreateTime：2013-11-18
	 */
	public String view() throws Exception{
		if((null != occupationalControlPlan)&&(null != occupationalControlPlan.getId()))
		{
			occupationalControlPlan = occupationalControlPlanService.getById(occupationalControlPlan.getId());
			String username = occupationalControlPlan.getUserName();
			...
			occupationalControlPlan.setUserName(username);
			if(occupationalControlPlan.getLinkId() != null && !"".equals(occupationalControlPlan.getLinkId()))
			{
				Map map = new HashMap();
				map.put("taskProId",occupationalControlPlan.getLinkId());
				map.put("picType","occupationalControlPlan");
	 ...
	 * Function Description：初始化职业病危害控制效果评价报告表备案通知书修改信息
	 * Writer：陆婷
	 * CreateTime：2013-11-18
	 */
	public String initEdit() throws Exception{
		if((null != occupationalControlPlan)&&(null != occupationalControlPlan.getId()))
		{
			...
			if(occupationalControlPlan.getLinkId() == null || "".equals(occupationalControlPlan.getLinkId()))
			{
				...
				occupationalControlPlan.setLinkId(linkId);
			}
			else
			{
				Map map = new HashMap();
		...
		else
		{
			...
			occupationalControlPlan.setSendTime(sdf.format(new Date()));
			String linkId = java.util.UUID.randomUUID().toString().replace("-", "");
			occupationalControlPlan.setLinkId(linkId);
			occupationalControlPlan.setUserName("       你单位                                                          提请备案的                                                      职业病危害控制效果评价报告表及文件、资料收悉。经核实，提交的文件、资料齐全，符合有关规定，现予以备案。");
		}
		String deptCode = this.getLoginUserDepartment().getDeptCode();
	...
	public String save() throws Exception{
		if(occupationalControlPlan.getQyid() != null && !"".equals(occupationalControlPlan.getQyid()))
		{
			Map map = new HashMap();
			map.put("companyId", occupationalControlPlan.getQyid());
			CompanyBackUp company = companyService.getCompanyBackupById(map);
			if(!occupationalControlPlan.getCompanyName().equals(company.getCompanyname()))
			{
				occupationalControlPlan.setQyid(null);
				Department dept = deptService.findDeptByDeptCode(occupationalControlPlan.getSzzid());
				occupationalControlPlan.setSzzid(dept.getDeptCode());
				occupationalControlPlan.setSzzname(dept.getDeptName());
			}
			else
			{
		...
		else if(occupationalControlPlan.getSzzid() != null && !"".equals(occupationalControlPlan.getSzzid()))
		{
			Department dept = deptService.findDeptByDeptCode(occupationalControlPlan.getSzzid());
			occupationalControlPlan.setSzzid(dept.getDeptCode());
			occupationalControlPlan.setSzzname(dept.getDeptName());
		...
		if ("add".equalsIgnoreCase(this.flag)){
			occupationalControlPlan.setDeptId(this.getLoginUserDepartmentId());
			occupationalControlPlan.setDelFlag(0);
			occupationalControlPlan.setCreateTime(new Date());
			occupationalControlPlan.setCreateUserID(this.getLoginUserId());
			occupationalControlPlanService.save(occupationalControlPlan);
		}else{
			occupationalControlPlanService.update(occupationalControlPlan);
		}
		return RELOAD;
	}
	...
	public String delete() throws Exception{
	    try{
			occupationalControlPlanService.deleteWithFlag(ids);
	...
	public String print() throws Exception{
		if((null != occupationalControlPlan)&&(null != occupationalControlPlan.getId()))
		{
			...
			map.put("requiredOne", isNull(occupationalControlPlan.getRequiredOne()));
			map.put("requiredTwo", isNull(occupationalControlPlan.getRequiredTwo()));
			map.put("requiredThree", isNull(occupationalControlPlan.getRequiredThree()));
	        map.put("companyName", isNull(occupationalControlPlan.getCompanyName()));
	        String username = isNull(occupationalControlPlan.getUserName());
	        int length = 1;
			if(username != null && !"".equals(username))
			{
				String[] s = username.split("\r\n");
	        ...
	        map.put("contact", isNull(occupationalControlPlan.getContact()));
	        map.put("tel", isNull(occupationalControlPlan.getTel()));
	        String sendTime = isNull(occupationalControlPlan.getSendTime());
			...
			root = root + "occupationalControlPlan.doc";
			
			HWPFDocument document= replaceDoc(root, map,length);
	        ByteArrayOutputStream ostream = new ByteArrayOutputStream();
		        try {
		              document.write(ostream);
		              Struts2Util.getResponse().addHeader("Content-Disposition", "attachment;filename=occupationalControlPlan.doc");
	...
	public void savePhoto(){
		try {
			...
			root = root.replaceAll("\\\\", "/");
			root = root.replace("webapps","virtualdir/upload/file/occupationalControlPlan/");
			...
			String picname = "occupationalControlPlan/"+imgName;
			taskPic.setPicName(picname);
			taskPic.setTaskProId(occupationalControlPlan.getLinkId());
			taskPic.setTaskRemark("");
			taskPic.setDelFlag(0);
			taskPic.setPicType("occupationalControlPlan");//设置图片类型 安全管理表
	...
	public OccupationalControlPlan getOccupationalControlPlan(){
		return this.occupationalControlPlan;
	}

	public void setOccupationalControlPlan(OccupationalControlPlan occupationalControlPlan){
		this.occupationalControlPlan = occupationalControlPlan;
	}
	...
	public String getSbrqStart() {
		return sbrqStart;
	}

	public void setSbrqStart(String sbrqStart) {
		this.sbrqStart = sbrqStart;
	}
	...
	public void setSbrqEnd(String sbrqEnd) {
		this.sbrqEnd = sbrqEnd;
	}
