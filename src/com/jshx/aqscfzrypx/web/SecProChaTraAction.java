package com.jshx.aqscfzrypx.web;
...
public class SecProChaTraAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private SecProChaTra secProChaTra = new SecProChaTra();

	/**
	 * 业务类
	 */
	@Autowired
	private SecProChaTraService secProChaTraService;
	@Autowired
	...
	public void list() throws Exception{
		...
		if(null != secProChaTra){
			...
			if ((null != secProChaTra.getAreaId()) && (0 < secProChaTra.getAreaId().trim().length())){
				paraMap.put("areaId", secProChaTra.getAreaId().trim());
			}
			...
			if ((null != secProChaTra.getTrainingPersonName()) && (0 < secProChaTra.getTrainingPersonName().trim().length())){
				paraMap.put("trainingPersonName", "%" + secProChaTra.getTrainingPersonName().trim() + "%");
			}
			
			if ((null != secProChaTra.getTrainingCardnum()) && (0 < secProChaTra.getTrainingCardnum().trim().length())){
				paraMap.put("trainingCardnum", "%" + secProChaTra.getTrainingCardnum().trim() + "%");
			}

			if ((null != secProChaTra.getCompanyName()) && (0 < secProChaTra.getCompanyName().trim().length())){
				paraMap.put("companyName", "%" + secProChaTra.getCompanyName().trim() + "%");
			}
			
			if ((null != secProChaTra.getCompanyId()) && (0 < secProChaTra.getCompanyId().trim().length())){
				paraMap.put("companyId",   secProChaTra.getCompanyId().trim()  );
			}
		...
		pagination = secProChaTraService.findByPage(pagination, paraMap);
	...
	public String view() throws Exception{
		if((null != secProChaTra)&&(null != secProChaTra.getId()))
		{
			secProChaTra = secProChaTraService.getById(secProChaTra.getId());
			if(secProChaTra.getLinkId() == null || "".equals(secProChaTra.getLinkId()))
			{
					String linkId = java.util.UUID.randomUUID().toString().replace("-", "");
					secProChaTra.setLinkId(linkId);
				}
				else
				{
					Map map = new HashMap();
	...
	public String initEdit() throws Exception{
		...
		secProChaTra.setCompanyId(enBaseInfo.getId());
	    return EDIT;
	}
	...
	public String save() throws Exception{
		...
		map.put("loginId", this.getLoginUser().getLoginId());
		EntBaseInfo enBaseInfo = entBaseInfoService.findEntBaseInfoByMap(map);
		secProChaTra.setAreaId(enBaseInfo.getEnterprisePossession());
		Map m = new HashMap();
		m.put("codeName", "企业属地");
		m.put("itemValue", enBaseInfo.getEnterprisePossession());
		secProChaTra.setAreaName(codeService.findCodeValueByMap(m).getItemText());
		secProChaTra.setCompanyId(enBaseInfo.getId());
		secProChaTra.setCompanyName(enBaseInfo.getEnterpriseName());

		if ("add".equalsIgnoreCase(this.flag)){
			secProChaTra.setDeptId(this.getLoginUserDepartmentId());
			secProChaTra.setDelFlag(0);
			secProChaTraService.save(secProChaTra);
		}else{
			secProChaTraService.update(secProChaTra);
		}
	...
	private void setBlobField(FileInputStream in)
	{
		if (null != secProChaTra)
		{
			try
			{
				//此处将File对象转换成blob对象，并设置到secProChaTra中去
	...
	public String delete() throws Exception{
	    try{
			secProChaTraService.deleteWithFlag(ids);
...
//        }
	...
	public SecProChaTra getSecProChaTra(){
		return this.secProChaTra;
	}

	public void setSecProChaTra(SecProChaTra secProChaTra){
		this.secProChaTra = secProChaTra;
	}
