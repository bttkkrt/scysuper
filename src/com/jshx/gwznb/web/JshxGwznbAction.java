/**
 * Class Name: JshxGwznbAction
 * Class Description：岗位和职能部门责任制
 */
package com.jshx.gwznb.web;
...
public class JshxGwznbAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private JshxGwznb jshxGwznb = new JshxGwznb();
	...
	public void list() throws Exception{
		...
		if(null != jshxGwznb){
			if ((null != jshxGwznb.getQymc()) && (0 < jshxGwznb.getQymc().trim().length())){
				paraMap.put("qymc", "%" + jshxGwznb.getQymc().trim() + "%");
			}
			if ((null != jshxGwznb.getSzc() )&& (0 < jshxGwznb.getSzc().trim().length())){
				paraMap.put("szc",jshxGwznb.getSzc().trim());
			}
		}
		...
		if ((null != jshxGwznb.getSzzid()) && (0 < jshxGwznb.getSzzid().trim().length())){
			paraMap.put("szzid",  jshxGwznb.getSzzid().trim() );
		}
	...
	public String view() throws Exception{
		if((null != jshxGwznb)&&(null != jshxGwznb.getId())){
			jshxGwznb = jshxGwznbService.getById(jshxGwznb.getId());
			if(jshxGwznb.getLinkId() == null || "".equals(jshxGwznb.getLinkId()))
			{
				String linkId = java.util.UUID.randomUUID().toString().replace("-", "");
				jshxGwznb.setLinkId(linkId);
			}
			else
			{
				Map map = new HashMap();
				map.put("taskProId",jshxGwznb.getLinkId());
				map.put("picType","gwznb");
	...
	public String save() throws Exception{
		...
		if ("add".equalsIgnoreCase(this.flag)){
			...
			jshxGwznb.setSzzid(dept.getDeptCode());
			jshxGwznb.setSzzname(dept.getDeptName());
			jshxGwznb.setQyid(company.getId());
			jshxGwznb.setQymc(company.getCompanyname());
			jshxGwznb.setDeptId(this.getLoginUserDepartmentId());
			jshxGwznb.setDelFlag(0);
			jshxGwznb.setCreateUserID(this.getLoginUserId());
			jshxGwznb.setCreateTime(new Date());
			jshxGwznb.setQylx(company.getQylx());
			jshxGwznb.setHyfl(company.getHyfl());
			jshxGwznb.setQygm(company.getQygm());
			jshxGwznb.setQyzclx(company.getQyzclx());

			jshxGwznb.setIfwhpqylx(company.getIfwhpqylx());
			jshxGwznb.setIfyhbzjyqy(company.getIfyhbzjyqy());
			jshxGwznb.setIfzywhqylx(company.getIfzywhqylx());
			jshxGwznb.setIffmksjyqy(company.getIffmksjyqy());//heyc 20141210 设置非煤矿山标识
			
			jshxGwznb.setSzc(company.getSzc());
			jshxGwznb.setSzcname(company.getSzcname());
			jshxGwznb.setCompanyType(company.getCompanyType());//hanxc 20141223 企业所属监管类型
			jshxGwznb.setIfzsqy(company.getIfzsqy());//hanxc 20141223 是否直属企业
			jshxGwznbService.save(jshxGwznb);
		}else{
			jshxGwznb.setCompanyType(company.getCompanyType());//hanxc 20141223 企业所属监管类型
			jshxGwznb.setIfzsqy(company.getIfzsqy());//hanxc 20141223 是否直属企业
			jshxGwznbService.update(jshxGwznb);
		}
		return RELOAD;
	}
	...
	public String delete() throws Exception{
	    try{
			jshxGwznbService.deleteWithFlag(ids);
	...
	public void savePhoto(){
		try {
			...
			root = root.replace("webapps","virtualdir/upload/file/gwznb/");
			...
			String picname = "gwznb/"+imgName;
			taskPic.setPicName(picname);
			taskPic.setTaskProId(jshxGwznb.getLinkId());
			taskPic.setTaskRemark("");
			taskPic.setDelFlag(0);
			taskPic.setPicType("gwznb");//设置图片类型 安全管理表
	...
	public JshxGwznb getJshxGwznb(){
		return this.jshxGwznb;
	}

	public void setJshxGwznb(JshxGwznb jshxGwznb){
		this.jshxGwznb = jshxGwznb;
	}
