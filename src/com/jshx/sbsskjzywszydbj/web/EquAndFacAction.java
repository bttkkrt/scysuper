package com.jshx.sbsskjzywszydbj.web;
...
public class EquAndFacAction extends BaseAction
{
	 ...
	 * 实体类
	 */
	private EquAndFac equAndFac = new EquAndFac();

	/**
	 * 业务类
	 */
	@Autowired
	private EquAndFacService equAndFacService;
	...
	public void list() throws Exception{
		...
		if(null != equAndFac){
			...
			if ((null != equAndFac.getAreaId()) && (0 < equAndFac.getAreaId().trim().length())){
				paraMap.put("areaId",  equAndFac.getAreaId() );
			}

			if ((null != equAndFac.getCompanyName()) && (0 < equAndFac.getCompanyName().trim().length())){
				paraMap.put("companyName", "%" + equAndFac.getCompanyName().trim() + "%");
			}

			if ((null != equAndFac.getEquipmentName()) && (0 < equAndFac.getEquipmentName().trim().length())){
				paraMap.put("equipmentName", "%" + equAndFac.getEquipmentName().trim() + "%");
			}

			if ((null != equAndFac.getEquipmentNumber()) && (0 < equAndFac.getEquipmentNumber().trim().length())){
				paraMap.put("equipmentNumber", "%" + equAndFac.getEquipmentNumber().trim() + "%");
			}
		...
		final String filter = "id|areaId|companyName|equipmentName|equipmentNumber|";
		...
		pagination = equAndFacService.findByPage(pagination, paraMap);
	...
	public String view() throws Exception{
		if((null != equAndFac)&&(null != equAndFac.getId())){
			equAndFac = equAndFacService.getById(equAndFac.getId());
			if(equAndFac.getMapkey() == null || "".equals(equAndFac.getMapkey()))
			{
				String mapKey = java.util.UUID.randomUUID().toString();
				equAndFac.setMapkey(mapKey);
			}
		}else{
			String mapKey = java.util.UUID.randomUUID().toString();
			equAndFac.setMapkey(mapKey);
	...
	public String save() throws Exception{
		...
		equAndFac.setAreaId(enBaseInfo.getEnterprisePossession());
		Map<String,Object> m = new HashMap<String,Object>();
		m.put("codeName", "企业属地");
		m.put("itemValue", enBaseInfo.getEnterprisePossession());
		equAndFac.setAreaName(codeService.findCodeValueByMap(m).getItemText());
		equAndFac.setCompanyId(enBaseInfo.getId());
		equAndFac.setCompanyName(enBaseInfo.getEnterpriseName());
		
		if ("add".equalsIgnoreCase(this.flag)){
			equAndFac.setDeptId(this.getLoginUserDepartmentId());
			equAndFac.setDelFlag(0);
			equAndFacService.save(equAndFac);
		}else{
			equAndFacService.update(equAndFac);
		}
		SaveOrUpdateDxx dot = new SaveOrUpdateDxx();
		//删除点位信息
		dot.saveDot("4", equAndFac.getMapkey(),equAndFac.getEquipmentLongitude(),equAndFac.getEquipmentLatitude());
		//保存点位信息
		dot.saveDot("1", equAndFac.getMapkey(), equAndFac.getEquipmentLongitude(),equAndFac.getEquipmentLatitude());
		return RELOAD;
	}
	...
	private void setBlobField(FileInputStream in)
	{
		if (null != equAndFac)
		{
			try
			{
				//此处将File对象转换成blob对象，并设置到equAndFac中去
	...
	public String delete() throws Exception{
	    try{
			equAndFacService.deleteWithFlag(ids);
...
//        }
	...
	public EquAndFac getEquAndFac(){
		return this.equAndFac;
	}

	public void setEquAndFac(EquAndFac equAndFac){
		this.equAndFac = equAndFac;
	}
